...
function checksubscribe(jsonResult)
{
	...
	if(jsonResult.ALLOWPLAY==false)
	{
	   ...
	   if(isselectplay){
	       ...
	       pageName="check/page/buy_discount.jsp?SUBJECTID=<%=typeid%>&VODID="+ data.SERIES[0].SERIEID +"&FATHERID=" + vodid  +"&PLAYTYPE=1&CONTENTTYPE="+data.CONTENTTYPE+"&BUSINESSTYPE=1&returnurl="+escape(location.href)+"&playurl="+escape(playname);		
					
	   }else{
	       playname="au_PlayFilm.jsp?PLAYTYPE=1&CONTENTTYPE=0&BUSINESSTYPE=1&ISVIP=1&PROGID="+tempdatalist[area1.curindex].SERIEID+
	                "&FATHERSERIESID=" +  vodid +"&returnurl="+escape(location.href);
		   pageName="check/page/buy_discount.jsp?SUBJECTID=<%=typeid%>&VODID="+ tempdatalist[area1.curindex].SERIEID +"&FATHERID=" + vodid  +"&PLAYTYPE=1&CONTENTTYPE="+data.CONTENTTYPE+"&BUSINESSTYPE=1&returnurl="+escape(location.href)+"&playurl="+escape(playname);
	  }
