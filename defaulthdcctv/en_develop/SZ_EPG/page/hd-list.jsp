...
window.onload = function(){	
	area0=AreaCreator(8,1,new Array(-1,-1,-1,3),"area0_list_","className:item item_select","className:item");
	...
	area0.areaInedEvent = function(){
		$("area0_list_3").className = "item item_select";
		}
	area0.areaOutedEvent = function(){
		$("area0_list_3").className = "item item_focus";
		}
	
	area1=AreaCreator(1,3,new Array(7,0,3,2),"area1_list_","className:item item_focus","className:item");
	...
	area3.changefocusedEvent = function(){
		var tempURL = "hd-list.jsp?sonTypeid="+typeid+"&typeid="+farTypeid+"&curpage="+curpage;
		area6.doms[0].updateimg(datalist[area3.curindex].PICURL);
		area6.doms[0].mylink = "vod_redirect.jsp?vodid="+datalist[area3.curindex].VODID+"&typename="+typename+"&returnurl="+escape(tempURL);
	}
...
//���ֶ�����ť�������ӡ�ѡ��Ч��
	area7=AreaCreator(1,1,new Array(-1,-1,1,-1),"jifen_","className:item item_focus","className:item");
        area7.doms[0].mylink = "../../jifen/index.jsp?returnUrl=../SZ_EPG/page/hd-list.jsp?typeid="+farTypeid;
	...
	pageobj = new PageObj(areaid!=null?parseInt(areaid):0,indexid!=null?parseInt(indexid):3, new Array(area0, area1, area2, area3, area4, area5, area6, area7));
	...
	pageobj.goBackEvent=function(){
		if(pageobj.curareaid == 2 || pageobj.curareaid == 5){
			$("area5_list_0").innerHTML = "";
			$("area2_list_0").innerHTML = "";
			}else{
...
function bindarea0data(num){
	...
	curpage = 1;
	dataFrame.location.href = "<%=path%>datajsp/hd-list_iframedata.jsp?typeid="+typeid+"&curpage=1";
	}
...
function initPage(){
	...
	if(pagecount == 1){
		...
		area4.doms[0].setCanFocus(false,2);
		area4.doms[1].setCanFocus(false,2);
		}else if(curpage == 1){
			area1.doms[0].setCanFocus(true);
			area4.doms[0].setCanFocus(true);
			area1.doms[1].setCanFocus(false,2);
			area4.doms[1].setCanFocus(false,2);
			}else if(curpage == pagecount){
				area1.doms[0].setCanFocus(false,1);
				area4.doms[0].setCanFocus(false,1);
				area1.doms[1].setCanFocus(true);
				area4.doms[1].setCanFocus(true);
				}else{
			area1.doms[0].setCanFocus(true);
			area1.doms[1].setCanFocus(true);
			area4.doms[0].setCanFocus(true);
	...
	if(datalist.length>0){
		...
		for(var i=0;i<area3.doms.length;i++){
			if(i<datalist.length){
				...
				area3.doms[i].setcontent("",datalist[i].VODNAME,44,true,true);
				area3.doms[i].mylink = "vod_redirect.jsp?vodid="+datalist[i].VODID+"&typeid="+typeid+"&returnurl="+escape(tempURL);
			}else{
...
function initData(){
	...
	if(datalist.length>0){
		if(area3.curindex>datalist.length-1 && pageobj.curareaid == 3){	
			pageobj.changefocus(3,0);
			$("area0_list_3").className = "item item_focus";
		}else if(area3.curindex>datalist.length-1 && pageobj.curareaid == 0){
			pageobj.changefocus(0,3);
			}
			if(area3.curindex>=datalist.length-1)
			{
				area3.curindex=datalist.length-1;
...
function loadIframeData(){
	dataFrame.location.href = "<%=path%>datajsp/hd-list_iframedata.jsp?typeid="+typeid+"&curpage="+curpage;
}
...
<div class="nav">
	...
	<div id="area0_list_3" class="item item_select" style="top:210px;">
