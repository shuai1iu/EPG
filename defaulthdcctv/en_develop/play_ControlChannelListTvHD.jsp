...
if(isfromgat==0){
  result = metaData.getChanListByTypeId("10000100000000090000000000039770",4,0);//高清直播CODE
...
String[] isTvod = new String[listSizeStr];
...
while(iter.hasNext())
{
	...
	*/
	...
	//String istvodFlag = (String)chanInfo.get("ISTVOD");
	isTvod[realSize] = "1";
...
//	isSub[realSize] = 1;// ((Integer)chanInfo.get("ChannelPurchased")).intValue();
	...
	//isTvod[realSize] = (String)chanInfo.get("ISTVOD");
	realSize++;
}
//判断频道是否加锁
...
%>
	...
	parent.isTvod[<%=i%>] = '<%=isTvod[i]%>'; 
	</script>		
<%        
 }
%>
<script>
parent.tuiJianDate=<%=tuijianjsonvodlist%>;
...
//当前是第几页
if(currIndex % 4 == 0){
	parent.currListPage = currIndex / 4;
}else{
	parent.currListPage = getInt(currIndex / 4);
}
parent.getDataFlag = "true";
parent.init_page();
if("true" == parent.getDataFlag){parent.showChannelList();}
