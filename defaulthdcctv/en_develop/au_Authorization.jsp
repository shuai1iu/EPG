...
String mProdPrice = "";
String mProdName = "";
ArrayList mProdIdList = new ArrayList();
ArrayList mProdPriceList = new ArrayList();
ArrayList mProdNameList = new ArrayList();
if(null != monthList && monthList.size()>0)
{
   ...
   {
        Object tempProdIdObject = monthList.get(i);
        Map tempMonthMap = null;
        tempMonthMap = (Map)tempProdIdObject;
        String tempProdId = (String) tempMonthMap.get("PROD_CODE");
        String tempProdPrice = (String) tempMonthMap.get("PROD_PRICE");
         String tempProdName = (String) tempMonthMap.get("PROD_NAME"); 
        mProdIdList.add(tempProdId);
        mProdPriceList.add(tempProdPrice);
        mProdNameList.add(tempProdName);
    }
   System.out.println("LISTAL====" + mProdIdList + "||===Priced=" + mProdPriceList + "||===Name=" + mProdNameList);
}
...
if( 1 != isVir)
{
	...
	else if (retCode == 0x07020002||retCode ==500||retCode ==501||retCode ==502||retCode ==503||retCode ==504)
	{
		...
		{
                     System.out.println("MPROD===" + mProdIdInt);
			//4200 420001  时尚
			if(mProdIdList.contains("4200")||mProdIdList.contains("420001"))
			{		
				turnPage.removeLast();
				...
				String play_url="../../au_PlayFilm.jsp?PLAYTYPE=1&CONTENTTYPE=0&BUSINESSTYPE=1&ISVIP=1&PROGID="+sProgId+
				 ...
				 pageName = "checkAuthorization/page/order_ss.jsp?SUBJECTID="+typesId+"&VODID="+sProgId+"&FATHERID="+sFatherSeriesId+"&PLAYTYPE=1&CONTENTTYPE="+sContentType+"&BUSINESSTYPE=1";
				 %>
				<script>
                var pageName =  "<%=pageName%>";
			...
			// 尊享 4201 420101  高清大片  4300   高清热剧  4301	高清纪实  4302	高清热剧  4303
			else if((mProdIdList.contains("4201")||mProdIdList.contains("420101"))&&(mProdIdList.contains("4300")||mProdIdList.contains("4301")
				||mProdIdList.contains("4302")||mProdIdList.contains("4303")))
			{
				turnPage.removeLast();
				String goback = turnPage.go(0);
				String play_url="../../au_PlayFilm.jsp?PLAYTYPE=1&CONTENTTYPE=0&BUSINESSTYPE=1&ISVIP=1&PROGID="+sProgId+
				 ...
				 pageName = "checkAuthorization/page/order_zx.jsp?SUBJECTID="+typesId+"&VODID="+sProgId+"&FATHERID="+sFatherSeriesId+"&PLAYTYPE=1&CONTENTTYPE="+sContentType+"&BUSINESSTYPE=1";
				 %>
				<script>
                var pageName =  "<%=pageName%>";
                window.location.href = pageName+"&playurl="+escape('<%=play_url%>&returnurl='+escape('<%=goback%>'))+"&returnurl="+escape("<%=goback%>");
			...
			else
			{
				...
				String play_url="../../au_PlayFilm.jsp?PLAYTYPE=1&CONTENTTYPE=0&BUSINESSTYPE=1&ISVIP=1&PROGID="+sProgId+
				 ...
				 pageName = "checkAuthorization/page/order_zz.jsp?SUBJECTID="+typesId+"&VODID="+sProgId+"&FATHERID="+sProgId+"&PLAYTYPE=1&CONTENTTYPE="+sContentType+"&BUSINESSTYPE=1";
				 %>
				<script>
                var pageName =  "<%=pageName%>";
                window.location.href = pageName+"&playurl="+escape('<%=play_url%>&returnurl='+escape('<%=goback%>'))+"&returnurl="+escape("<%=goback%>");
