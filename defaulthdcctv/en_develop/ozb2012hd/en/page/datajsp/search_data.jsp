...
<%
	 ...
	 //获取热词
	 ArrayList hotkey_list = (ArrayList)metadata.getTypeListByTypeId(Index_Hotkey,3,0);
...
function bindSearchResultData(itms){
	if(itms!=undefined&&itms!="null"){
		area6.setpageturndata(itms.length,resultpagecount);
		for(var i=0,l=area6.doms.length,dl=itms.length;i<l;i++){
			if(i<dl){
				area6.doms[i].setcontent("",(i+1)+"."+itms[i].vodName,59);
				area6.doms[i].mylink = "vod-detail.jsp?vodid="+itms[i].vodId+"&returnurl="+escape("search.jsp?keyword="+keyword+"&hotkeyword="+hotkeyword+"&returnurl="+returnurl);
			}else{
				area6.doms[i].updatecontent("");
				area6.doms[i].mylink = "";
			}
		}
		$('result_page').innerHTML = area6.curpage+"/"+(resultpagecount==undefined?1:resultpagecount)+"页";
	}else{
		area6.datanum = 0;
