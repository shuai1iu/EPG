...
function MediaPlayerExClass() {
    ...
    this.initMediaPlay=function( ) {
         ...
         this.left = 0;
         this.top = 0;
         this.nativeUIFlag = 1; // 0:不使用盒子UI 1:使用盒子UI
         this.muteFlag = 1; //0: 不使用盒子静音 1:使用盒子静音      
         this.subtitleFlag = 1; // 0:不显示字幕  1:显示字幕    
    	 this.videoAlpha = 0; // 透明度 0：不透明 255:完全透明 透明度根据情况0~255    
    	 this.cycleFlag = 0;
    	 this.randomFlag = 0;
    	 this.autoDelFlag = 0;
    ...
    this.playSet=function(BookMark) {
		...
		this.mp.playByTime(1,BookMark,1); // playByTime(1：正常播放时间NPT 2:clock time,书签点,速度)
		}
   ...
   this.setCurrChannelIndex= function (offset){
    ...
    this.getDateByUTC=function(utcstr) {
		...
		if(curUTCStr)
		{
			...
			var dateStr = year + "/" + month + "/" + day + " "  + hoursD8 + ":" + minutes + ":" + seconds;
