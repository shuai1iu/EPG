...
.control_panel .timeSzmgstyle{ position:absolute; top:35px; left:505px; width:300px; text-align:center}
.control_panel .timeSzmgstyleSpeed{ position:absolute; top:35px; left:770px; width:300px; text-align:center}
...
.control_panel .bar3 { position:absolute; top:70px; left:190px; height:32px; padding-right:12px}
.control_panel .bar3{ background:url(images/playerimages/progressbar02.png) right no-repeat;}
.control_panel .bar2{ position:absolute; top:70px; left:190px; background:url(images/progressbar.png) repeat; height:32px;}
.control_panel .bar2_focus{ position:absolute; top:70px;background:url(images/progressbar_focus.png) no-repeat; height:32px; width:32px;}
...
var timeForwardErrorDIvIsShow = false;//广电不可跳广告错误 hhr
...
function pressOk()
{
	if(playStat == "fastforward"){$("speedDiv").style.display = "none"; //隐藏进度条
	speedDivIsDispaly  = false;playStat = "play";return;}
	...
	else if(timeForwardErrorDIvIsShow)
	{
		$("timeForwardErrorDIv").style.display = "none";
		timeForwardErrorDIvIsShow = false ;
	}
	return;
}
...
{
	if(playStat == "fastforward"){
		$("speedDiv").style.display = "none";//隐藏进度条
...
function fastRewind()
{
	...
	$("seekDiv").style.display = "none"; 
	seekDivIsShow = false;
    if(playStat == "play" || playStat == "fastforward"||(speed >= 32 && playStat == "fastrewind")){ speed = 1;}
	speed = speed * 2;
	playStat = "fastrewind";
	mp.fastRewind(-speed);
...
function displaySeekTable(playFlag)
{
	...
	{	
		...
		jumpPos=4;
		//$("currentTime_progress").style.background="url(images/playerimages/progressbar1.png)";
		$("seekPosBut").style.display="block";
...
//判断是否大约当前时间，用于深圳广电集团频道快进屏蔽 hhr
function isForwardCurrTime(pHour, pMin)
{
	var tmpCurrTime = parseInt(mp.getCurrentPlayTime());
	pHour = pHour.replace(/^0*/, "");
	if(pHour == ""){pHour = "0";}
	pMin = pMin.replace(/^0*/, "");        
	if(pMin == ""){pMin = "0";}
	var alltime=pHour*3600+pMin*60;
	//alert("alltime="+alltime+" tmpCurrTime="+tmpCurrTime);
...
function clickJumpBtn()
{
	...
	if(!checkJumpTime(inputValueHour, inputValueMin))
	{
		...
		clearTimeout(timeID_jumpTime);timeID_jumpTime = "";
		//timeID_jumpTime = setTimeout(hideJumpTimeDiv,15000);
	}else if(!isForwardCurrTime(inputValueHour, inputValueMin))
	{
		$("timeForwardErrorDIv").style.display = "block";
		timeForwardErrorDIvIsShow = true ;
		$("jumpTimeHour").innerHTML = "";
		$("jumpTimeMin").innerHTML = "";
		preInputValueHour = "";
		preInputValueMin = "";	
...
function processSeek(_currTime)
{
	...
	var currlegth=(2 + currCellCount * 9);
	$("seekPos").style.width = currlegth + "px";
	$("seekPosBut").style.left = (180+currlegth) + "px";
...
{	
	...
	if(playStat == "fastrewind"){mp.resume();}
	$("speedDiv").style.display = "block"; //显示进度条
	speedDivIsDispaly  = true;
	fastTime = setTimeout(hiddenFastDiv, 5000);
	playStat = "fastforward";
	$("statusImg").innerHTML = '<img src="images/playerimages/icon_fast.png" align="absmiddle"/>&nbsp;';
}
function hiddenFastDiv()
{
...
function jumpPressKeyRight()
{
	...
	else if(jumpPos==2)
	{
		...
		//no response for SZMG request. HHR
	}
	/*
...
function goPreProg()
{
	...
	+  "&PROGSTARTTIME=" + preProgBeginTime +"&PROGENDTIME=" + preProgEndTime + "&CHANNELID=" + channelId;
	setEPG();
	mp.stop();
...
function goNextProg()
{
	...
	+  "&PROGSTARTTIME=" + nextProgBeginTime +"&PROGENDTIME=" + nextProgEndTime + "&CHANNELID=" + channelId;
	setEPG();
	mp.stop();
	window.location.href = jumpUrl;
}
...
<div id="bottomframe" style="position:absolute;left:30px; top:530px; width:600px; height:150px; z-index:1">
</div>
...
<div id="speedDiv" class="control_panel control_panel3" style="display:none">
    <div id="szmgInfoSpeed" class="timeSzmgstyleSpeed">此频道不支持快进操作</div>
    <div id="currTime" class="playing_time" style="top:35px;">01:29</div>
...
<div id="seekDiv" style="display:none">  
  <div class="control_panel">
  		<div id="szmgInfo" class="timeSzmgstyle">此频道不支持快进操作</div>
		...
		<div class="bar2" id="seekPos" style="width:20px"></div>
		<div class="bar2_focus" id="seekPosBut" style="left:200px;"></div>
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" style="font-size:38px; color:#FFFFFF">
        ...
        <tr >
        <td width="20"></td>
        <td height="125" align="center" style="font-size:28px; color:#FFFFFF" id="infoText">此频道输入时间大于当前播放时间，请重新输入！</td>
        <td width="20"></td>
        </tr>
...
<!----------时间输入错误提示----------->
<!---------广电频道不可跳转广告错误提示------------>
<div id="timeForwardErrorDIv" style="background:url(images/playerimages/popup_bg.png) no-repeat left;position:absolute;left:300px;top:170px;width:720px;height:380px;display:none;">
	<table border="0" cellspacing="0" cellpadding="0" width="95%" style="font-size:38px; color:#FFFFFF">
		<tr>
			<td height="115" align="center" >温馨提示</td>
		</tr>
		<tr >
