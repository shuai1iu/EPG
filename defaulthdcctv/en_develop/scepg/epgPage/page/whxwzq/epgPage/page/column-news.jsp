...
String categoryName="最新闻联播";
String categoryCode="10000100000000090000000000031172";
...
<%
 JSONObject tempjsontypeList = (JSONObject)request.getAttribute("colNewsVodList");
/* int temptotalSize=Integer.parseInt(tempjsontypeList.get("totalSize").toString());
 int tempcurPage=Integer.parseInt(tempjsontypeList.get("curPage").toString());
 int temptotalPage=Integer.parseInt(tempjsontypeList.get("totalPage").toString());*/
 JSONArray  tempVodList=(JSONArray)tempjsontypeList.get("vodDataList");
...
function callVodListData(data)
{
	...
	//	$("area1_txt_"+i).innerHTML=data.vodDataList[i].name;
		if(getbytelength(vodListData.vodDataList[i].name)>=12)
		{
			vodListData.vodDataList[i].iscut=true;
			vodListData.vodDataList[i].cutname=getcutedstring(vodListData.vodDataList[i].name,12,false);
		}else
		{
...
{
	//获取playUrl进行播放
	var curDivindex=area1.curindex%10;
	...
	area1SelectIndex=area1.curindex%10;
	var name=vodListData.vodDataList[area1SelectIndex].name;
	$("newVodname").innerHTML=name.length>10?name.substr(0,8)+"...":name;
...
{	
	...
	{   
		...
		initArea1Select();
		$("getColumnNewsFrame").src="iframe/getColumnNewsFrame.jsp?categoryCode=10000100000000090000000000031172&pageSize=10&curpage="+area1.curpage;
	}
}
...
{
	mp.stop();
	var contentCode=vodListData.vodDataList[area1.curindex%10].contentCode;
	var programCode=vodListData.vodDataList[area1.curindex%10].programCode;
	$("getVodPlayUrlFrame").src="iframe/getVodPlayUrlFrame.jsp?programCode="+programCode+"&contentCode="+contentCode;
...
{
	playUrl=url;
    playUrl = "rtsp://192.168.2.109/12.ts";
...
<div class="wrapper">
	<!-- head -->
	<div id="categoryName" class="txt" style="color:#000;font-size:27px;font-weight:bold;left:39px;top:53px;width:560px;">名栏-新闻联播</div>
	<!-- head the end -->
	...
	<div class="txt" style="color:#000; font-size:18px; left:229px; top:102px; text-align:right; width:100px;" id="curpgTotalpg">1/2页</div>
	<!-- page the end-->
		...
		<div class="item" id="area1_div_0">
			...
			<div class="txt" id="area1_txt_0">
                    <% if(tempVodList.size()>0) {
					   if(getbytelength(tempVodList.getJSONObject(0).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(0).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:32px;" id="area1_div_1">
			...
			<div class="txt" id="area1_txt_1">
                    <% if(tempVodList.size()>1) {
					   if(getbytelength(tempVodList.getJSONObject(1).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(1).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:64px;"  id="area1_div_2">
			...
			<div class="txt" id="area1_txt_2">
                    <% if(tempVodList.size()>2) {
					   if(getbytelength(tempVodList.getJSONObject(2).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(2).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:96px;" id="area1_div_3">
			...
			<div class="txt" id="area1_txt_3">
                  <% if(tempVodList.size()>3) {
					   if(getbytelength(tempVodList.getJSONObject(3).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(3).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:128px;" id="area1_div_4">
			...
			<div class="txt" id="area1_txt_4">
                    <% if(tempVodList.size()>4) {
					   if(getbytelength(tempVodList.getJSONObject(4).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(4).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:160px;" id="area1_div_5">
			...
			<div class="txt" id="area1_txt_5">
                   <% if(tempVodList.size()>5) {
					   if(getbytelength(tempVodList.getJSONObject(5).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(5).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:192px;" id="area1_div_6">
			...
			<div class="txt" id="area1_txt_6">
                   <% if(tempVodList.size()>6) {
					   if(getbytelength(tempVodList.getJSONObject(6).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(6).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:224px;" id="area1_div_7">
			...
			<div class="txt" id="area1_txt_7">
                    <% if(tempVodList.size()>7) {
					   if(getbytelength(tempVodList.getJSONObject(7).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(7).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:256px;" id="area1_div_8">
			...
			<div class="txt" id="area1_txt_8">
                   <% if(tempVodList.size()>8) {
					   if(getbytelength(tempVodList.getJSONObject(8).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(8).get("name").toString(),12,false)%>
					<%}else{%>
		...
		<div class="item" style="top:288px;" id="area1_div_9">
			...
			<div class="txt" id="area1_txt_9">
                    <% if(tempVodList.size()>9) {
					   if(getbytelength(tempVodList.getJSONObject(9).get("name").toString())>=12){
						  %>
						   <%=getcutedstring(tempVodList.getJSONObject(9).get("name").toString(),12,false)%>
					<%}else{%>
