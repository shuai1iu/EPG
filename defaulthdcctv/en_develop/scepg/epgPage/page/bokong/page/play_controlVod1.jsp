...
//	playUrl = serviceHelpHWCTC.getTriggerPlayUrlHWCTC(iPlayType,iProgId,iPlayBillId,beginTime,endTime,productId,serviceId,contentType);
    ...
    System.out.println(">>>>>>>>>>>>>playUrl>>>>>>>>>11111111111"+playUrl);
}
...
if(playUrl != null && playUrl.length() > 0)
{
	...
	if(-1 != tmpPosition)
	{
	    ...
	    System.out.println(">>>>>>>>>>>>>playUrl>>>>>>>>>222"+playUrl);
...
if(!isSucess)
{
	System.out.println(">>>>>>>>>>>>>playUrl>>>>>>>>>222"+isSucess);
...
function pressOk()
{
	...
	else if(minEPGIsShowFlag==true)
	{
		...
		{
			unload();
			window.location.href="play_controlVod1.jsp?PROGID="+preProgId+"&PLAYTYPE=1"+"&TYPE_ID="+"<%=typeId%>";
		}
		else
		{
			unload();
			window.location.href="play_controlVod1.jsp?PROGID="+nextProgId+"&PLAYTYPE=1"+"&TYPE_ID="+"<%=typeId%>";
		}
	}
	else if(seekDivIsShowFlag==true)
	{
...
function antoQuit()
{
	window.location.href ="column-news.jsp";
}
...
function loadData()
{
	$("getDataIframe").src = "play_controlVodData.jsp?progId=1579865&categoryCode="+"<%=typeId%>";
}
