...
<%
	...
	/////////////////////////////////////////////////////////////
	List markArray = Arrays.asList("0-0","0-1","1-0","1-1"); //判断区域此标签为标清本地需显示的
	...
	if(szOriResultList!=null&&szOriResultList.size()>1){
		ParseResultList=(ArrayList)szOriResultList.get(1);
	}
	if(allOriResultList!=null&&allOriResultList.size()>1){
		tempAllList = (ArrayList)allOriResultList.get(1);
		if(tempAllList!=null)	ParseResultList.addAll(tempAllList);
	}
	...
	for(int i=0;i<dianbocodes.length;i++)
	{ 
		String[] res=dianbocodes[i].split("==");
		String Name=res[0];
		String Value=res[1];
		HashMap map=new HashMap();
		map.put("TYPE_NAME",Name);
		map.put("TYPE_ID",Value);
		newresultList.add(map);
	}
	*************************/
	ArrayList resultList=new ArrayList();
	HashMap mcount=new HashMap();
	mcount.put("COUNTTOTAL",dianbocodes.length);
