...
<%
	...
	ArrayList testList = metadata.getTypeListByTypeId("10000100000000090000000000035632",0,10);
	//10000100000000090000000000035632
	...
	//增值业务栏目
	ArrayList arrayVAS = metadata.getVasListByTypeId("10000100000000090000000000038252",10,0);


	System.out.println("=============================================arrayVAS====================================="+arrayVAS);
	HashMap count2 = (HashMap)arrayVAS.get(0);
	ArrayList arrayVASList = (ArrayList)arrayVAS.get(1);
	for(int i=0;i<(Integer)count2.get("COUNTTOTAL");i++)
	{
			HashMap tempNewMap = (HashMap)arrayVASList.get(i);
			tempNewMap.put("ISVAS",1);
			tempNewMap.put("VAS_ID",(Integer)tempNewMap.get("VASID"));
			tempNewMap.put("TYPE_NAME",(String)tempNewMap.get("VASNAME"));
                      //  tempNewMap.put("TYPE_NAME","");
			tempNewMap.put("URL",serviceHelp.getVasUrl((Integer)tempNewMap.get("VASID")).toString());
			System.out.println("====================================URL======================="+tempNewMap.get("URL"));
			fullTypeList.add(tempNewMap);
	...
	if(ParseResultList!=null&&ParseResultList.size()>0){
		for(int i=0;i<ParseResultList.size();i++){
			...
			if(introduce!=null&&markArray.contains(introduce)){
				...
				resultMap.put(("ISVAS"),0);//是否是vas的标识位 0为不是vas 1说明条目是vas
				  ...
				  area1.doms[i].setcontent("",itms[i].TYPE_NAME,13);
				   ...
				   if(itms[i].ISVAS==0)
				       ...
				       area1.doms[i].mylink = tempstr+"?typeid=" + itms[i].TYPE_ID+"&returnurl="+escape(location.href); 
						...
						{
					//	alert(itms);
					   	area1.doms[i].mylink = "../../"+itms[i].URL;
					//	alert(1);
						}
				}
                else{
