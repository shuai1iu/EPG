...
<%
	...
	ArrayList tempDataList = (ArrayList)uvr.getViewingRecordList(vodid,contentType,1,0);
	if(tempDataList!=null&&tempDataList.size()>1){
		ArrayList recordList = (ArrayList)tempDataList.get(1);
		if(recordList.size()>0){
			isRecord = 1;
			HashMap recordMap = (HashMap)recordList.get(0);
			breakpoint = (String)recordMap.get("BREAKPOINT");
		}
	}
%>
...
<script type="text/javascript">
	...
	window.onload=function(){
		...
		{
			if(area0.curindex == 0 && hasRecord==1)
			{
				saveFocstr(pageobj);
				var play_url = "../au_PlayFilm.jsp?PROGID="+datajson[0].VODID;
				  play_url+="&FATHERSERIESID="+datajson[0].VODID;
				  play_url+="&CONTENTTYPE="+datajson[0].CONTENTTYPE;
				  play_url+="&BREAKPOINT="+breakpoint;
				  play_url+="&BUSINESSTYPE=1";
				  play_url+="&PLAYTYPE=1&ISVIP=1";
				  play_url+="&returnurl="+escape("<%=basePath%>film_details.jsp?vodid="+datajson[0].VODID+"&returnurl="+returnurl);
