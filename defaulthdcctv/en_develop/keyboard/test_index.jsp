...
<%
	...
	//	tag=0;
	Integer tag=1;
	System.out.println("==================================begin01-----------------------------------------------------");
	ArrayList tmpAdVas = new ArrayList();
		System.out.println("==================================begin-----------------------------------------------------");
		try{
			tmpAdVas=(ArrayList)new MetaData(request).getVasListByTypeId("10000100000000090000000000035250",6,0);
	...
	if(tmpAdVas!=null&&tmpAdVas.size()>0)
		...
		if(zt!=null&& zt.size()>0)
				...
				System.out.println("=============================posterpath======================="+apppic);
		}
	//}
	//tag++;
...
function foo()
	  ...
	  function iFrameHeight() 
		{ 	
			...
			//如果进入则先清空
			var imgpic = ifm.document.getElementsByTagName("img")[0].src;
			alert(imgpic);
			//var pic = imgpic[0].src;
			ifm.document.body.innerHTML="";
			
			ifm.height = parseInt(window.document.documentElement.clientHeight)+"px";
			ifm.width = parseInt(window.document.documentElement.clientWidth)+"px";
			window.document.getElementById("test").innerHTML=ifm.height+"+"+ifm.width;
			
			
			//alert(ifm.height);
			
			var ifm1 = window.document.getElementById("frame_content").contentWindow;
			var img = ifm1.document.createElement("img");
			//img.setAttribute("src","http://125.88.70.16:8082/EPG/jsp/defaulthdcctv/en_develop/images/myBgSD.jpg");
			img.setAttribute("src",imgpic);
			
			img.setAttribute("width","100%");
			img.setAttribute("height","100%");
			img.setAttribute("position","fixed");
			ifm.document.body.insertBefore(img);
...
window.onload=function()
{
	//广告加载部分的代码
			...
			if(tag==1&&apppic!=null&&apppic!=""&&apppic!=undefined)
					{
					
						
						document.getElementById("test").innerHTML=mp1.getVolume();
						mp1.setMuteFlag(1);
					   // document.getElementById("frame_content").src="images/myBgSD.jpg"; 
					
						document.getElementById("index").style.display="none";
					
						f = window.setTimeout("foo()",5000);
...
<!--20130909 10:00 ZSZ 修改背景图显示规避用户重启前不能重新加载背景图图片-->
 <div id="ad" >
	<iframe id="frame_content"  onload="iFrameHeight()" scrolling="no" frameborder="0" src="<%=apppic%>"  style="margin:0px;padding:0px;" >
	</iframe>
 </div>
<div id="index">
