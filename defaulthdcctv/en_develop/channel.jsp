...
<%
MetaData metaDataNum = new MetaData(request);
String introduce ="";
//HashMap typeinfomap = metaDataNum.getTypeInfoByTypeId("10000100000000090000000000028210"); //过滤不能直播不能快进频道
HashMap typeinfomap = metaDataNum.getTypeInfoByTypeId("10000100000000090000000000041210");
if(typeinfomap!=null){
...
<script type="text/javascript"> 
	var controlflag = 0;//0:正常频道 1：受控频道，只能向后移动进度条，不能向前移动（可以按返回键返回直播）。不可倍速快进、只能退。不可输入时间进行快速定位。
	var markArray = new  Array();//用来存放过滤的数据
	var markArrayTest = "<%=introduce %>";
	<%
	...
	%>
	function containFun(strArr,str)
	{	
		if(strArr!=null){
	...
	window.onload=function()
	{
		...
		area0.doms[1].mylink="jpui/page/newJpChannel.jsp";
		...
		area1.areaOkEvent=function()
		{
			...
			//alert(markArrayTest + markArray);
			if(containFun(markArray,channelNumber))
			{
				//alert("**channel**play_ControlChannel_sz***CHANNELNUM*****"+area1.doms[area1.curindex].channelnum);
				window.location.href="au_PlayFilm.jsp?PROGID=" + channelAuthorID + "&PLAYTYPE=2&CONTENTTYPE=3&&BUSINESSTYPE=2&ALLOWAD=0&CHANNELNUM="+channelNumber+"&COMEFROMFLAG=1&returnurl="+escape(location.href);
				//window.location.href="play_ControlChannel_sz.jsp?CHANNELNUM="+channelNumber+"&COMEFROMFLAG=1&returnurl="+escape(location.href);
			}
			else
				...
				//window.location.href="play_ControlChannel.jsp?CHANNELNUM="+channelNumber+"&COMEFROMFLAG=1&returnurl="+escape(location.href);
		...
		area2.areaOkEvent=function()
		{
			if(area2.curpage==1&&area2.curindex==0)
			{
				...
				//alert("**channel***CHANNELNUM*****"+area1.doms[area1.curindex].channelnum);
				var channelNumber2 = area1.doms[area1.curindex].channelnum;
				var channelAuthorID2 = area1.doms[area1.curindex].dataurlorparam;
				
				if(containFun(markArray,channelNumber2)){
					window.location.href="au_PlayFilm.jsp?PROGID=" + channelAuthorID2 + "&PLAYTYPE=2&CONTENTTYPE=3&&BUSINESSTYPE=2&ALLOWAD=0&CHANNELNUM="+channelNumber+"&COMEFROMFLAG=1&returnurl="+escape(location.href);
					//window.location.href="play_ControlChannel_sz.jsp?CHANNELNUM="+channelNumber2+"&COMEFROMFLAG=1&returnurl="+escape(location.href);
				}else{
					...
					//window.location.href="play_ControlChannel.jsp?CHANNELNUM="+channelNumber2+"&COMEFROMFLAG=1&returnurl="+escape(location.href);
				}
			}
		}
