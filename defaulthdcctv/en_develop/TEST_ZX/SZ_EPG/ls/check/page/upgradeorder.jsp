...
window.onload = function(){
   ...
   area0.doms[0].domOkEvent=function(){
	   window.location.href="order-sure.jsp?price=<%=ssprice%>&prodname=<%=ssproname%>&prodid=<%=ssprodid%>&serverid=<%=ssservercode%>&vodid=<%=ssvod%>&parentvodid=<%=ssvod%>&url1="+escape(window.location.href)+"&playurl="+escape("<%=playurl%>");
   };
