	     ...
	     if(!this.areas[this.curareaid].goback())
		 {
	         ...
	         if(this.backurl!=undefined)
			 {
				if(this.backurl.indexOf('?')==-1)
	                this.backurl+="?back=1";
                else if(this.backurl.indexOf('&back=1')==-1)
	                this.backurl+="&back=1"; 
	...
	this.changefocus=function(focusornot,backnochanggedata)
	{
		if(focusornot)
	   {
		   ...
		   if(this.cutcontent!=undefined)
            /*      ZTE
             *    首页--回放，左边的列表，当焦点在某一行时，滚动的字幕被遮住了一部分
             */
//	        this.updatecontent("<marquee style='height:34px'>"+this.content+"</marquee>");
//	        this.updatecontent("<marquee behavior='scroll' height='42' width='90%' startposition='90%'>"+this.content+"</marquee>");
			this.updatecontent("<marquee behavior='scroll' width='90%' startposition='90%'>"+this.content+"</marquee>");
		   //聚焦事件
		   if(!!this.focusEvent)
		      this.focusEvent();
	   }
		...
		for(j=0;j<newstyle.length;j++)
	   {
		   ...
		   if(tmpstyle!=undefined)
	      {
		   if(tmpstyle.indexOf(',')==-1)
		   {
             ...
             */
//			 var tempdisplay;
             var tempdisplay; if(this.dom[j] == undefined) continue;
			 if(this.dom[j] == undefined) continue;
			 if(this.dom[j].style.visibility=="hidden"){
					tempvisibility = "hidden";
			 }
				if(this.dom[j].style.display=="none"){
					tempdisplay = "none";
			 }
...
function keypress(keyval)
{
	switch(keyval)
	{
		...
		case KEY_0:
		    pageobj.numType(0);
		    window.location.href = "jpui/page/newJpChannel.jsp";
            break;
		case 49:
		case KEY_1:
		    pageobj.numType(1);
