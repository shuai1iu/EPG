...
<%
	...
	{
		//电影订购
	    retMap = serviceHelpHWCTC.authorizationHWCTC(sProgId,sPlayType, sContentType, sBusinessType,"-1",sFatherSeriesId);
	}
	else
	{
		//连续剧订购
		retMap = serviceHelpHWCTC.authorizationHWCTC(sFatherSeriesId,sPlayType, sContentType, sBusinessType,"-1",sFatherSeriesId);
	}
	...
	else
	{
			...
			else if(retCode == 504)//用户没有订购相应产品或订购关系已失效或未生效
			{
				...
				if(timeList!=null && timeList.size()>0)
				{
					...
					for(int i=0;i<timeList.size();i++){
						...
						timeresultMap.put("PROD_NAME",((HashMap)timeList.get(i)).get("PROD_NAME"));
						...
						timeresultMap.put("PROD_CODE",sProductCode);
						timeresultMap.put("SERVICE_CODE",sServiceId);
						timeresultMap.put("PROD_PRICE",(Integer.parseInt(sPrice)/100)+"");
		...
		+ endTime_onceArr.substring(6,8)+ " " +  endTime_onceArr.substring(8,10) + ":" + endTime_onceArr.substring(10,12);
				        timeresultMap.put("EXPIRE_TIME",endTime_once);
						temptimeList.add(timeresultMap);
					}
					resultMap.put("timeList",temptimeList);
				}
				if(mouseList!=null && mouseList.size()>0)
				{
					ArrayList tempmouseList = new ArrayList();
					for(int i=0;i<mouseList.size();i++){
						HashMap mouseresultMap = new HashMap();
						mouseresultMap.put("PROD_NAME",((HashMap)mouseList.get(i)).get("PROD_NAME"));
						...
						mouseresultMap.put("PROD_CODE",sProductCode);
						mouseresultMap.put("SERVICE_CODE",sServiceId);
						mouseresultMap.put("PROD_PRICE",(Integer.parseInt(sPrice)/100)+"");
						String endTime_onceArr = serviceHelp.getServiceExpireTime(sProductCode,sServiceId);
				        String endTime_once = endTime_onceArr.substring(0,4) + "-" + endTime_onceArr.substring(4,6) + "-" 
		+ endTime_onceArr.substring(6,8)+ " " +  endTime_onceArr.substring(8,10) + ":" + endTime_onceArr.substring(10,12);
				        mouseresultMap.put("EXPIRE_TIME",endTime_once);
						tempmouseList.add(mouseresultMap);
					}
					resultMap.put("mouseList",tempmouseList);
				}
				if(pointList!=null && pointList.size()>0)
				{
					ArrayList temppointList = new ArrayList();
					for(int i=0;i<pointList.size();i++){
						HashMap pointresultMap = new HashMap();
						pointresultMap.put("PROD_NAME",((HashMap)pointList.get(i)).get("PROD_NAME"));
						sProductCode = (String) ((HashMap)pointList.get(i)).get("PROD_CODE");
						sServiceId = (String) ((HashMap)pointList.get(i)).get("SERVICE_CODE");
						sPrice = (String) ((HashMap)pointList.get(i)).get("PROD_PRICE");
						if(sPrice == null || "".equals(sPrice))
						{
							sPrice = "0";
						}
						pointresultMap.put("PROD_CODE",sProductCode);
						pointresultMap.put("SERVICE_CODE",sServiceId);
						pointresultMap.put("PROD_PRICE",(Integer.parseInt(sPrice)/100)+"");
						String endTime_onceArr = serviceHelp.getServiceExpireTime(sProductCode,sServiceId);
				        String endTime_once = endTime_onceArr.substring(0,4) + "-" + endTime_onceArr.substring(4,6) + "-" 
		+ endTime_onceArr.substring(6,8)+ " " +  endTime_onceArr.substring(8,10) + ":" + endTime_onceArr.substring(10,12);
				        pointresultMap.put("EXPIRE_TIME",endTime_once);
						temppointList.add(pointresultMap);
					}
					resultMap.put("pointList",temppointList);
				}
