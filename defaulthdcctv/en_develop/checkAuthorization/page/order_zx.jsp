...
<%
   ...
   if(sProgId!=sFatherSeriesId){
	...
	if(tempmouseList!=null  && tempmouseList.size()>0){
		...
		if(monthcode1.equals("420101")||monthcode1.equals("4201")){
			zxname=monthname1;
			zxsj=monthsj1;
			zxprice=monthprice1;
	...
	if(tempmouseList!=null  && tempmouseList.size()>1){
		...
		if(monthcode2.equals("420101")||monthcode2.equals("4201")){
			zxname=monthname2;
			zxsj=monthsj2;
			zxprice=monthprice2;
				...
				area0.doms[0].domOkEvent=function(){
					if(<%=checkresult%>==1)
					{
					   window.location.href="order-sure.jsp?isppv=1&price=<%=ppvprice%>&prodname=<%=ppvname%>&prodid=<%=ppvcode%>&serverid=<%=ppvservicecode%>&vodid=<%=sProgId%>&parentvodid=<%=fatherSeriesId%>&url1="+escape(window.location.href)+"&playurl="+escape("<%=playurl%>");
						...
						window.location.href="order_ss.jsp?SUBJECTID=<%=typesId%>&VODID=<%=sProgId%>&FATHERID=<%=fatherSeriesId%>&PLAYTYPE=<%=sPlayType%>&CONTENTTYPE=<%=sContentType%>&BUSINESSTYPE=<%=sBusinessType%>&returnurl="+escape(window.location.href)+"&playurl="+escape("<%=playurl%>");
					}
				};
			}else{
				area0.doms[0].domOkEvent=function(){
...
<div class="wrapper">
    ...
    <div class="j-recommend j-recommendB">
		<div class="j-bg"></div>
		<div class="icon" style="left:1157px; top:382px;"><img src="../images/icon-hot.png" /></div>
        <div class="txt" style=" left:864px; top:446px; color: #b24509; font-size: 38px ; font-weight: bold; text-align:center; width:350px;"><%=columnpname%></div>
