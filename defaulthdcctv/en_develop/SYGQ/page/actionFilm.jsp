...
<%@ include file="datajsp/actionFilmData.jsp"%>
...
window.onload = function()
{
	...
	area2.areaOkEvent = function()
	{
		...
		{
			window.location.href="detail.jsp?vodid="+temppageTurnList[area2.curindex].VODID+"&cateNamePosition=1&returnurl="+escape(location.href)+"&typeid=<%=dongzuo%>";
			saveFocstr(pageobj);
		}
	}
	...
	area2.asyngetdata=function()
	{
		//turnpageShowData(this.curpage)
		 $('hidden_frame').src = "<%=basePath%>datajsp/actionFilmData.jsp?isFirstFlag=1&stratPosition="+(this.curpage-1)*8;
	...
	area1.stablemoveindex = new Array("0-1,1-1,2-1,3-1",-1,-1,-1);
	area1.areaOkEvent = function()
	{
		...
		else if(area1.curindex==3)
		{
			   ...
			   &&area1_numString>0&&area1_numString<=area2.pagecount)
			{
			    ...
			    $('hidden_frame').src = "<%=basePath%>datajsp/actionFilmData.jsp?isFirstFlag=1&stratPosition="+(area2.curpage-1)*8;
			}
			else
			{
				area1_numString="";
	...
	pageobj = new PageObj(areaid!=null?parseInt(areaid):0,indexid!=null?parseInt(indexid):1, new Array(area0,area1,area2,area3,area4),null);
...
function bindFilmsData(datavalue)
{
	...
	else
	{
		if(area2.curpage == 1)
		{
			...
			area3.doms[1].setCanFocus(false,2);
			//area3.doms[1].setCanFocus(false,2);
			//$("area1_list_1").className="item item_not";
			//pageobj.changefocus(1,1);
			//pageobj.changefocus(3,1);
		/*	$("area1_list_1").className="item item_not";
			$("area3_list_1").className="item item_not";*/
...
<!--head-->
	...
	<!--menu-->
		<div class="menu">
        	<div class="item item_select" style="z-index:-2;left:152px;"></div>
			<!--焦点 
				class="item item_focus"
				选中
