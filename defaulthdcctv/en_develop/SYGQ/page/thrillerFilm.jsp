...
<%@ include file="datajsp/thrillerFilmData.jsp"%>
...
window.onload = function()
{
	...
	area2.areaOkEvent = function()
	{
		...
		{
			window.location.href="detail.jsp?vodid="+temppageTurnList[area2.curindex].VODID+"&cateNamePosition=4&returnurl="+escape(location.href)+"&typeid=<%=jingsong%>";
			saveFocstr(pageobj);
		}
	}
	...
	area2.asyngetdata=function()
	{
		...
		$('hidden_frame').src = "<%=basePath%>datajsp/thrillerFilmData.jsp?isFirstFlag=1&stratPosition="+(this.curpage-1)*8;
		//getAJAXData("datajsp/thrillerFilmData.jsp?stratPosition="+(this.curpage-1)*8+"&isFirstFlag=1",bindFilmsData);
		
	}
	
	area1 = AreaCreator(1,4,new Array(0,-1,2,-1),"area1_list_","className:item item_focus","className:item");
	area1.stablemoveindex = new Array("0-4,1-4,2-4,3-4",-1,-1,-1);
	area1.areaOkEvent = function()
	{
		...
		else if(area1.curindex==3)
		{
			   ...
			   &&area1_numString>0&&area1_numString<=area2.pagecount)
			{
			    ...
			    $('hidden_frame').src = "<%=basePath%>datajsp/thrillerFilmData.jsp?isFirstFlag=1&stratPosition="+(area2.curpage-1)*8;
			}
			else
			{
				area1_numString="";
	...
	pageobj = new PageObj(areaid!=null?parseInt(areaid):0,indexid!=null?parseInt(indexid):4, new Array(area0,area1,area2,area3),null);
	...
	{
	    bindFilmsData(thrillerFilmData);
	}
}
...
<body>
	...
	<!--menu-->
		<div class="menu">
        	<div class="item item_select" style="z-index:-2;left:608px;"></div>
			<!--焦点 
