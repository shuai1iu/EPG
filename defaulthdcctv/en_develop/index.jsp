...
/*存储探针所需内容名称 20140210 LS*/
var imgName = new Array();
var changeFlag = 3;
var changeRecommend =  window.setInterval("changeImage()",5000);
/*isFirst为标明首页是否是开机第一次访问*/
var isFirst = <%=isFirst%>;
function pcmouseover(tempareaid,tempindexid){
	pageobj.changefocus(tempareaid,tempindexid);
}

function pcok(tempareaid,tempindexid){
	pageobj.changefocus(tempareaid,tempindexid);
	pageobj.ok();
}
...
/*当首页为开机第一次访问时，触发开机探针*/
    if("1" == isFirst)
    {
        $("log_frame").src = "http://219.133.42.99:8080/EPG_Statistics/EPG_BOOT.jsp?userID="+user+"&STB_IP="+ STB_IP+"&EPG_TYPE=HD&STB_TYPE="+ STBType + "&GROUP=" + <%=groupId%>;
    }
...
//	area0.doms[7].mylink=indexhref[7];
	...
	area0.setfocuscircle(0);
        ...
        {
		           /*在首页左侧导航栏点击确认键时触发入口探针*/
                   $("log_frame").src = "http://219.133.42.99:8080/EPG_Statistics/EPG_AREA.jsp?userID="+user+"&EPG_TYPE=HD&EPG_CATA=EPG_HD_CATA_LEFT_"+area0.curindex;
...
//中间视频
	...
	{
	    /*在中间视频大窗点击确认键时触发入口探针*/
	    $("log_frame").src = "http://219.133.42.99:8080/EPG_Statistics/EPG_AREA.jsp?userID="+user+"&EPG_TYPE=HD&EPG_CATA=EPG_HD_CATA_MEIDA_0";
	}
	if(area1setlink == 0)
 ...
 function changeImage()
{
    ...
    if( pageobj.curareaid == 5)
    {
		...
		else
	    {
			...
			getdatacommon(list3rd_right3,2,1,0,vasList3rd_right3Pic);
            getdatacommon(list3rd_right4,3,1,0,vasList3rd_right4Pic);
		    changeFlag = 1;
    ...
    }else
    {
	    ...
	    {
	         $("log_frame").src = "http://219.133.42.99:8080/EPG_Statistics/EPG_AREA.jsp?userID="+user+"&EPG_TYPE=HD&EPG_CATA_NAME="+imgName[area3.curindex]+"&EPG_CATA=EPG_HD_CATA_RIGHT_"+(area3.curindex+8);
	    }
            getdatacommon(list3rd_right1,0,1,0,vasList3rd_right1Pic);
            getdatacommon(list3rd_right2,1,1,0,vasList3rd_right2Pic);
			getdatacommon(list3rd_right3,2,1,0,vasList3rd_right3Pic);
    ...
    area3.areaOkEvent = function()
    {
        $("log_frame").src = "http://219.133.42.99:8080/EPG_Statistics
	/EPG_AREA.jsp?userID="+user+"&EPG_TYPE=HD&EPG_CATA_NAME="+imgName[area3.curindex]+"&EPG_CATA=EPG_HD_CATA_RIGHT_"+(area3.curindex+((log_changeFlag-1)*4));
    }
}
			...
			for(var i = start;i < end ; i++)
			{
				if((i-start)<data.length)
				{
					...
					area3.doms[i].mylink="vod_turnpager.jsp?vodid="+data[i-start].VODID+"&typeid=<%=shouyetuijian2code%>"+"&returnurl="+returnurl;
                                        imgName[i] = data[i -start].VODNAME;
			...
			for(var i = start; i < end ; i++)
			{
				if(i<area3.datanum)
				{
					...
					area3.doms[i].mylink = "../../" + data[i-start].VASURL;
                                        imgName[i] = data[i -start].VASNAME;
				}
				else
				{
					area3.doms[i].updateimg("#");
...
function getdataSecond(four,hdtwo)
{
	...
	area4.areaOkEvent = function()
       {
            request_log(area4,four,"DOWN",(area4.curindex+4));
       }
...
function getdata(five,sdtwo)
{
	...
	{
	    request_log(area2,sdtwo,"DOWN",area2.curindex);
	}
}

/*
    @decription 针对首页下部文字推荐位触发探针所用方法，由于下部文字推荐位在前期设计时涉及区域较多较复杂，因此使用下述方法
	@author LS
	@Date 2014-02-14
*/
function request_log (area,dataArray,dir,num)
{
      var showName = encodeURI(area.doms[area.curindex].contentdom.innerText);
      $("log_frame").src = "http://219.133.42.99:8080/EPG_Statistics/EPG_AREA.jsp?userID="+user+"&EPG_TYPE=HD&EPG_CATA=EPG_HD_CATA_"+dir+"_"+num+"&EPG_CATA_NAME="+showName+"&EPG_CATA_VASID="+dataArray[area.curindex];
...
<div class="menuli" id="area0_list_0" style="top:1px"  onClick="pcok(0,0);"  onMouseOver="pcmouseover(0,0);"><img src="images/icon_0.png" />首  页</div> <!--class="menuli on"-->
<div style="top:55px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_1" style="top:56px" onClick="pcok(0,1);"  onMouseOver="pcmouseover(0,1);"><img src="images/icon_1.png" />频  道</div>
<div style="top:110px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_2" style="top:111px" onClick="pcok(0,2);"  onMouseOver="pcmouseover(0,2);"><img src="images/icon_2.png" />点  播</div>
<div style="top:165px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_3" style="top:166px" onClick="pcok(0,3);"  onMouseOver="pcmouseover(0,3);"><img src="images/icon_3.png" />专  题</div>
<div style="top:220px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_4" style="top:221px" onClick="pcok(0,4);"  onMouseOver="pcmouseover(0,4);"><img src="images/icon_4.png" />回  放</div>
<div style="top:275px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_5" style="top:276px" onClick="pcok(0,5);"  onMouseOver="pcmouseover(0,5);"><img src="images/icon_5.png" />深  圳</div>
<div style="top:330px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_6" style="top:331px" onClick="pcok(0,6);"  onMouseOver="pcmouseover(0,6);"><img src="images/icon_11new.png" />排  行</div>
<div style="top:385px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_7" style="top:386px" onClick="pcok(0,7);"  onMouseOver="pcmouseover(0,7);"><img src="images/icon_7.png" />应  用</div>
<div style="top:440px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_8" style="top:441px" onClick="pcok(0,8);"  onMouseOver="pcmouseover(0,8);"><img src="images/icon_9.png" />空  间</div>
<div style="top:499px"><img src="images/menu_line.png" /></div>
<div class="menuli" id="area0_list_9" style="top:500px" onClick="pcok(0,9);"  onMouseOver="pcmouseover(0,9);"><img src="images/icon_10.png" />搜  索</div>
...
-->
<div id="area2_list_0" class="item item04" onClick="pcok(2,0);"  onMouseOver="pcmouseover(2,0);">
<div id="area2_listName_0" class="txt">直播港澳台</div>
</div>
<div id="area2_list_1" class="item item03" style="left:169px;" onClick="pcok(2,1);"  onMouseOver="pcmouseover(2,1);">
<div class="icon"></div>
<div id="area2_listName_1" class="txt">尊享包</div>
</div>
<div id="area2_list_2" class="item item03" style="left:293px;" onClick="pcok(2,2);"  onMouseOver="pcmouseover(2,2);">
<div id="area2_listName_2" class="txt">时尚包</div>
</div>
<div id="area2_list_3" class="item item03" style="left:418px;" onClick="pcok(2,3);"  onMouseOver="pcmouseover(2,3);">
<div id="area2_listName_3" class="txt">淘淘乐园</div>
</div>

<div id="area4_list_0" class="item item02" style="top:44px;"  onClick="pcok(4,0);"  onMouseOver="pcmouseover(4,0);">
<div id="area4_listName_0" class="txt">看大片</div>
</div>
<!------area4_list0 and area4_list_9,left:84px----->
<div id="area4_list_1" class="item item02" style="left:84px;top:44px;"   onClick="pcok(4,1);"  onMouseOver="pcmouseover(4,1);">
<div id="area4_listName_1" class="txt"style="text-align:center;">热剧</div>
</div>
<div id="area4_list_2" class="item item01" style="left:169px;top:44px;"  onClick="pcok(4,2);"  onMouseOver="pcmouseover(4,2);">
<div id="area4_listName_2" class="txt">热剧</div>
</div>
<div id="area4_list_3" class="item item01" style="left:232px;top:44px;"  onClick="pcok(4,3);"  onMouseOver="pcmouseover(4,3);">
<div id="area4_listName_3" class="txt">热剧</div>
</div>
<div id="area4_list_4" class="item item01" style="left:293px;top:44px;"  onClick="pcok(4,4);"  onMouseOver="pcmouseover(4,4);">
<div id="area4_listName_4" class="txt">热剧</div>
</div>
<div id="area4_list_5" class="item item01" style="left:357px;top:44px;"  onClick="pcok(4,5);"  onMouseOver="pcmouseover(4,5);">
<div id="area4_listName_5" class="txt">热剧</div>
</div>
<div id="area4_list_6" class="item item01" style="left:418px;top:44px;"  onClick="pcok(4,6);"  onMouseOver="pcmouseover(4,6);">
<div id="area4_listName_6" class="txt">热剧</div>
</div>
<div id="area4_list_7" class="item item01" style="left:480px;top:44px;"  onClick="pcok(4,7);"  onMouseOver="pcmouseover(4,7);">
<div id="area4_listName_7" class="txt">热剧</div>
</div>

<div id="area4_list_8" class="item item02" style="top:85px;"  onClick="pcok(4,8);"  onMouseOver="pcmouseover(4,8);">
<div id="area4_listName_8" class="txt">纪实</div>
</div>
<div id="area4_list_9" class="item item02" style="left:84px;top:85px;"   onClick="pcok(4,9);"  onMouseOver="pcmouseover(4,9);">
<div id="area4_listName_9" class="txt" style="text-align:center;">娱乐</div>
</div>
<div id="area4_list_10" class="item item01" style="left:169px;top:85px;"  onClick="pcok(4,10);"  onMouseOver="pcmouseover(4,10);">
<div id="area4_listName_10" class="txt">热剧</div>
</div>
<div id="area4_list_11" class="item item01" style="left:232px;top:85px;"  onClick="pcok(4,11);"  onMouseOver="pcmouseover(4,11);">
<div id="area4_listName_11" class="txt">热剧</div>
</div>
<div id="area4_list_12" class="item item01" style="left:293px;top:85px;"  onClick="pcok(4,12);"  onMouseOver="pcmouseover(4,12);">
<div id="area4_listName_12" class="txt">热剧</div>
</div>
<div id="area4_list_13" class="item item01" style="left:357px;top:85px;"  onClick="pcok(4,13);"  onMouseOver="pcmouseover(4,13);">
<div id="area4_listName_13" class="txt">热剧</div>
</div>
<div id="area4_list_14" class="item item01" style="left:418px;top:85px;"  onClick="pcok(4,14);"  onMouseOver="pcmouseover(4,14);">
<div id="area4_listName_14" class="txt">热剧</div>
</div>
<div id="area4_list_15" class="item item01" style="left:480px;top:85px;"  onClick="pcok(4,15);"  onMouseOver="pcmouseover(4,15);">
...
<!--隐藏iframe用于发送探针信息--->
<iframe id="log_frame" name = "log_frame" src="http://219.133.42.99:8080/EPG_Statistics/EPG_APP.jsp?userID=<%=userId%>&EPG_TYPE=HD&EPG_CATA_NAME=APP_HD_INDEXPAGE" style="width:1px;height:1px;display:none"></iframe>
