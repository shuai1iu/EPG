...
function displaySeekTable(playFlag)
{
	...
	if(isSeeking == 0)
	{	
		isSeeking = 1;  jumpDivIsShow = true;
...
{
	if(quitDivIsShow||nextDivIsShow){mainPressKeyLeft();}
...
{
	if(quitDivIsShow||nextDivIsShow){mainPressKeyRight();}
...
{
	if(!quitDivIsShow && !jumpToChannelInfoIsShow && !oneKeySwitchJumpInfoIsShow && !finishedDivIsShow && !nextDivIsShow)
	{
		if(!dataIsOk){return true;}
		if(minEpgIsShow){hideMinEpg();}
		else{showMinEpg();}
	...
	else if(quitDivIsShow||nextDivIsShow)
	{
		mainPressKeyOk();
		return true;
	}
...
function mainPressKeyLeft()
{
	if(positionFlag == 1)
	{
		...
		if(quitDivIsShow)
			$("quit").focus();
		if(nextDivIsShow){
			$("jumpOK").className = "item item_focus";
			$("jumpBack").className = "item";
		}
	}
}
...
function mainPressKeyRight()
{
	if(positionFlag == 0)
	{
		...
		if(quitDivIsShow)
			$("cancel").focus();
		if(nextDivIsShow){
			$("jumpOK").className = "item";
			$("jumpBack").className = "item item_focus";
		}
	}
}
...
var jumpCount = 5;//自动跳转计时器
function showNextDiv()
{
	nextDivIsShow = true;
	$("nextSureDiv").style.display = "block";
	if(jumpCount>0){
		$("counter").innerHTML = jumpCount;
		jumpCount--;
...
function goPreProg()
{
	...
	var jumpUrl="play_controlTVodJP.jsp?PROGID=" + preProgId + "&PLAYTYPE=4&CONTENTTYPE=300&&BUSINESSTYPE=5&PROGSTARTTIME=" + (startDate+nextProgBeginTime) + "&PROGENDTIME=" + (startDate+nextProgEndTime) + "&ISSUB=1&PREVIEWFLAG=1&TVOD=1&CHANNELID=" + channelId +"&LOGICCHANNELID="+chanNum+"&returnurl="+goBackUrl;
	mp.stop();
	//setEPG();
...
function goNextProg()
{
	var jumpUrl="play_controlTVodJP.jsp?PROGID=" + nextProgId + "&PLAYTYPE=4&CONTENTTYPE=300&&BUSINESSTYPE=5&PROGSTARTTIME=" + (startDate+nextProgBeginTime) + "&PROGENDTIME=" + (startDate+nextProgEndTime) + "&ISSUB=1&PREVIEWFLAG=1&TVOD=1&CHANNELID=" + channelId +"&LOGICCHANNELID="+chanNum+"&returnurl="+goBackUrl;
	mp.stop();
	//setEPG();
	window.location.href = jumpUrl;
}
...
<!--自动跳转弹出层-->
<div id="nextSureDiv" class="popup-lb-warp" style="display:none;">
	<div class="popup-lb">
		<div class="txt txt-font">是否播放下一个回看节目？<br /><span id="counter">5</span>秒后将自动播放</div>
		<div class="btn">
			<!--焦点 
					class="item item_focus"
			-->
			<div id="jumpOK" class="item item_focus">
				<div class="icon icon01"></div>
				<div class="txt">确&nbsp;&nbsp;定</div>
			</div>
			<div id="jumpBack" class="item" style="left:442px;">
