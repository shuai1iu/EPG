...
<script type="text/javascript">
        ...
        window.onload = function() {
            ...
            area0 = AreaCreator(2, 1, new Array(-1, -1, -11, 1), "menu", "className:menuli on", "className:menuli");
		   ...
		   area5.doms[0].domOkEvent = function(){ //书签播放
				location.href = "au_PlayFilm.jsp?PROGID="+jsonSitcoms[area1.curindex].VODID+"&PLAYTYPE="+EPGConstants.PLAY_TYPE_BOOKMARK+"&CONTENTTYPE="+jsonContent.CONTENTTYPE+"&BUSINESSTYPE="+EPGConstants.BUSINESS_TYPE_VOD+"&BEGINTIME="+begintime+"&ENDTIME="+endtime+"&TYPE_ID=<%=typeId%>&returnurl="+escape(location.href);
		   }
	   
		   area5.doms[1].domOkEvent = function(){ //从头播放
				location.href = "play_turnpager.jsp?vodid="+jsonSitcoms[area1.curindex].VODID+"&returnurl="+escape(location.href);
				popup3.closeme();
		   }
		   ...
		   popup3.goBackEvent=function()
		   {
			...
			area1.areaOkEvent = function(){
				//location.href = "vod_turnpager.jsp?vodid="+jsonContent.VODID+"&returnurl="+escape(location.href);
				//$("hidden_frame").src = "datajsp/check_bookmark.jsp?vodid="+jsonSitcoms[area1.curindex].VODID;
				 getAJAXData("datajsp/check_bookmark.jsp?curpage=" + this.curpage + "&cateid=<%=typeId%>&curindex="+area1.curindex,GetBookJson);
            ...
            area0.areaOkEvent = function() {
                switch (this.curindex) {
					...
					//case 0: break;
					case 0: window.location.href = "program_tv.jsp?typeid=<%=typeId%>&ptypeid=<%=parent_typeId%>&returnurl="+escape(returnurl); return true; break;  
                    case 1:	pageobj.changefocus(1,0);area0.setdarknessfocus(1);return true; break;
                    case 2: if(!isfaved){
            ...
            area1.asyngetdata = function(dataurl) {
                getAJAXData("datajsp/program_tvpart_choose_data_Ifrom.jsp?curpage=" + this.curpage + "&cateid=<%=typeId%>&curindex="+area1.curindex,GetJson);
			//alert("next");
            }
			...
			area1.changefocusedEvent = function(){
				getAJAXData("datajsp/program_tvpart_choose_data_Ifrom.jsp?ccid="+jsonSitcoms[area1.curindex].VODID,GetJson);
				//alert("qiehuan");
   ...
   function getbookmark(){
	   if(isbookmark==undefined){
		   ...
		   if(isbookmark==true){  //判断是否有书签
				...
				//window.alert();
				location.href = "play_turnpager.jsp?vodid="+jsonSitcoms[area1.curindex].VODID+"&returnurl="+escape(location.href);		
				//location.href = "au_PlayFilm.jsp?PROGID="+jsonContent.VODID+"&PLAYTYPE="+EPGConstants.PLAY_TYPE_VOD+"&CONTENTTYPE="+jsonContent.CONTENTTYPE+"&BUSINESSTYPE="+EPGConstants.BUSINESS_TYPE_VOD+"&TYPE_ID=<%=typeId%>&returnurl="+escape(location.href);
			}
	   }
   }
    </script>