...
<%@ include file="datajsp/vodThirdData.jsp"%>
<%
...
var cruntTypePage = <%=request.getParameter("curpage")==null?1:Integer.parseInt(request.getParameter("curpage"))%>;
var cruntpage=1;
...
window.onload = function()
{
	  if(returnurl.indexOf('back=1')==-1)
	  {
	   ...
	   returnurl=escape(returnurl);
		...
		{
			backfocus=getCurFocus("vodThirdindex");
			...
			cruntTypePage=backfocus[1].curpage;
		}
		else
		{
			area0.curindex=indexid;
			area0.curpage=cruntTypePage;
		}
	  ...
	  pageobj.pageOkEvent=function()
	  {
		...
		saveCookie("vodThirdindex",json!=undefined?json:""); 
	  
	  }
	  
	  pageobj.goBackEvent=function()
	  {
		 window.location.href="vodSecond.jsp?back=1&typeid="+fatherTypeid+"&cateName="+fatherTypeName;
	  }
	  $("cateName").innerHTML="点播-"+titleName;
	 turnpageTypesShowData(cruntTypePage,typeData);
...
function bindFilmsData(datavalue,contentTotalList)
{
	...
	for(i=0;i<area1.doms.length;i++)
	{
	   if(i<datavalue.length)
	   {
		   ...
		   {
		       area1.doms[i].mylink="vod-tv-detail.jsp?catename="+titleName+"&vodid="+datavalue[i].VODID+"&returnurl="+returnurl;
		   }
		   else if(datavalue[i].ISSITCOM==1)
		   {
			   area1.doms[i].mylink = "vod-zt-detail.jsp?catename="+titleName+"&vodid="+datavalue[i].VODID+"&returnurl="+returnurl;
		   }
	   }
	   else
	   {
...
<div class="wrapper">
	...
	<!--<div id="testText" style="position:absolute; color:#F00; left: 195px; top: 32px; width: 415px; height: 33px;">1111</div>-->
