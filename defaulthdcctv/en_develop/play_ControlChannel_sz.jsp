...
HashMap typeinfomap = metaData.getTypeInfoByTypeId("10000100000000090000000000041210");
if(typeinfomap!=null){
...
.channel_choose{ position:absolute;left:0px;top:20px; background:url(images/playerimages/channel_OSD_lbg.png) repeat-y; width:405px; height:720px;}
...
var markArrayNUM = new  Array();//用来存放过滤的数据
	<%
		for(int i = 0;i < strarray.length;i++){
	%>
		markArrayNUM[<%=i %>] = "<%=strarray[i] %>";
	<%
	}
	%>
	
	//alert(markArrayNUM + '赋值');
...
{	
	if(playStat == "fastforward"){playStat = "play";$("speedDiv").style.display = 'none';speedDivIsShow = false;fastFowordNUM = 0;return;}
	//时移状态: 1激活 0去激
...
{    	
	...
	if(playStat == "play" || playStat == "fastforward" || (playStat == "fastrewind" && speed >= 32)) {speed = 1;}
	speed = speed * 2;
...
{ 		    
	...
	fastTime = setTimeout(hiddenFast,5000);
...
{	
	...
	else
	{
		...
		//alert(markArrayNUM + 'joinChannel');
		if(containFun(markArrayNUM,chanNum) && controlflag == 1){
			mp.joinChannel(chanNum);
			}else{
			window.location.href="play_ControlChannel.jsp?CHANNELNUM="+chanNum+"&COMEFROMFLAG=1&returnurl="+escape("channel.jsp");
			}
...
function arrowDown()
{
	...
	{
		$("seekPos").style.background = "url(images/playerimages/progressbar02.png) right";//进度点失去焦点的时候
		seekDIvPos=0;
		$("hour").className = "inp inp-focus";
		return;
	}
...
function resetSeekDiv()
{
	...
	$("seekPos").style.background = "url(images/playerimages/progressbar.png) right";//进度点得到焦点的时
}
//直播列表下键切直播 函数中注意先后顺序
...
<div id="speedDiv" class="control_panel control_panel3" style="display:none;">
    ...
    <div id="currTime" class="playing_time" style="top:35px;"></div>
