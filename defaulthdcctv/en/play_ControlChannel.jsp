...
String introduce = "";
//HashMap typeinfomap = metaData1.getTypeInfoByTypeId("10000100000000090000000000028210"); //过滤不能直播不能快进频道
HashMap typeinfomap = metaData1.getTypeInfoByTypeId("10000100000000090000000000041210"); 
if(typeinfomap!=null){
  introduce = (String)typeinfomap.get("TYPE_INTRODUCE");
}
String[] strarray1 = introduce.split("@");
...
var markArrayNUM = new  Array();//用来存放过滤的数据
	...
	%>
		markArrayNUM[<%=i %>] = "<%=strarray1[i] %>";
	<%
	}
	%>
	//alert(markArrayNUM + '正常频道赋值');
        ...
        //    if(isSubSingle==1&&currChannelIndex==61||isSubSingle==1&&currChannelIndex==65||isSubSingle==1&&currChannelIndex==63){
            //     mp.joinChannel(chanNum);
             ...
             if(isSub[currChannelIndex]==1 ){
				 if(!containFun(markArrayNUM,chanNum) && controlflag == 0){
					  mp.joinChannel(chanNum);      
					  $('tipText').style.visibility = "hidden"; 
				}
				else{
								window.location.href="play_ControlChannel_sz.jsp?CHANNELNUM="+chanNum+"&COMEFROMFLAG=1&returnurl="+backUrl;
				}
			 }
            else{
                  mp.stop();
...
function init_info(jsontext){
	...
	for(var i=0;i<thisjson.progMenus.length;i++)
	{
		document.getElementById("progName_"+i).innerText=thisjson.progMenus[i].progName;
...
<!--直播节目单OSD-->
<div id="filmInfo1" style="position:absolute; left:920px; top:0px; width:360px; height:720px;display:none;">
    <div class="channel_msg">
		<!--20130905 17:30 ZSZ 修改频道OSD样式-->
	    <div class="num" id ="num" style="position:absolute;font-size:30px;top:20px;left:60px;"></div>
