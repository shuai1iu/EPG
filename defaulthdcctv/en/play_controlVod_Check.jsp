...
if (request.getParameter("PRODUCTNAME") != null && !"".equals(request.getParameter("PRODUCTNAME")) && !"null".equals(request.getParameter("PRODUCTNAME"))) {
	String tempVal = URLDecoder.decode(request.getParameter("PRODUCTNAME"));
	vodName = new String(tempVal.getBytes("ISO-8859-1"), "UTF-8");
	try{
		String[] tmpStr = queryStr.split("&");
		StringBuffer sb = new StringBuffer();
		for (int i = 0 ;i < tmpStr.length ; i++){
			if(tmpStr[i].contains("PRODUCTNAME") || tmpStr[i].contains("PRODUCTPRICE") || tmpStr[i].contains("VIPPRODUCT")){
				continue;
			}else{
				sb.append(tmpStr[i]);
				sb.append("&");
		...
		String subStr1 = "深圳IP电视";
		String subStr2 = "在线";
		if(vodName.contains(subStr1)){
			vodName = vodName.substring(subStr1.length(),vodName.length());
		}
		if(vodName.contains(subStr2)){
			vodName = vodName.substring(0,vodName.indexOf(subStr2));
		}
	}catch(Exception e){

	}
}
if (request.getParameter("PRODUCTPRICE") != null && !"".equals(request.getParameter("PRODUCTPRICE")) && !"null".equals(request.getParameter("PRODUCTPRICE"))) {
	vodPrice = Integer.parseInt(request.getParameter("PRODUCTPRICE"));
}
int filmId = 0;
try{
...
Map vodMap = metaData.getVodDetailInfo(filmId);
if ( vodMap != null && vodMap.size() != 0 ){
	//海报信息
	HashMap picpathTemp = (HashMap)vodMap.get("POSTERPATHS"); 
	picPath = getPosterPath(picpathTemp,request);
}
}catch(Exception e){
	System.out.println("AAAAA");
}
%>
var flag = 0;//0表示返回，1表示确认
document.onkeypress = eventHandler;
var picpath = '<%=picPath%>';
var returnurl = '<%=returnUrl%>';
var vodname = '<%=vodName%>';
var vodprice = <%=vodPrice%>/100;
function check(){
	flag = 1;
	displayBuy();
	if(picpath != "false"){
		document.getElementById('img_0').style.backgroundImage = "url(" + picpath + ")";
	}
	if(vodname != "false"){
		document.getElementById('vodname').innerHTML = vodname ;
	}
	if(vodprice != 0){
		document.getElementById('vodprice').innerHTML = vodprice + "元/月";
	}
...
function displayBack(){
		setWebkitTransform("choice_back", "VodCheck_back_on.png");
		setWebkitTransform("choice_buy", "VodCheck_buy_off.png");
}
function displayBuy(){
		setWebkitTransform("choice_back", "VodCheck_back_off.png");
		setWebkitTransform("choice_buy", "VodCheck_buy_on.png");
}
...
<body bgcolor="transparent" leftmargin="0px" topmargin="0px" marginwidth="0px" marginheight="0px" style="background-color: transparent" onLoad="start();init();check();" onUnload="unload()">

<!--鉴权失败弹出的提示框-->
<div id="choices" style="visibility:visible; width:1280px; height:720px; position:absolute; z-index:21; background:url('images/play_controlVod_Prompt.png') no-repeat;">
	<div class = "images" id = "img_0" style = " position:absolute; z-index:12; width:256px; height:339px; top:167px; left:178px; "></div>
	<div class = "vodname" id ="vodname" style=" position:absolute; z-index:12; width:300px; height:40px; top:275px; left:480px; text-align:center; overflow:hidden;">获取不到产品包</div>
	<div class = "vodprice" id="vodprice" style=" position:absolute; z-index:12; width:300px; height:40px; top:275px; left:750px; text-align:center;">获取不到价格</div>
	<div class="button" id="choice_buy" style=" position:absolute; z-index:12; width:266px; height:77px; top:435px; left:635px; background:url('images/VodCheck_buy_off.png') no-repeat;"></div>
	<div class="button" id="choice_back" style="position:absolute; width:266px; height:77px; top:435px; left:845px; background:url('images/VodCheck_back_off.png') no-repeat; "></div>
</div>
<!--鉴权失败弹出的提示框-->

<!---����----->
<div id="showMessageDiv" style="float:left; background-color:#999; height:80px; width:420px; margin-left:355px; margin-top:30px;text-align:center;z-index:4; display:none;">
   	<span id="testMsg" style="color:#000; font-size:28px; font-weight:bold;">��Ƶ��Ϊ����Ƶ����ĿǰΪ��������ڣ���ʽ�շѽ���ǰ��֪��</span>
</div>
