    ...
    else if ( filmInfoMap != null && filmInfoMap.size() != 0 )
    {
			...
			if(allActor!=null && allActor.size() != 0)
			{
                ...
                if(actors!=null && actors.length!=0)
				{
					...
					System.out.println(actorsLength+"::::::::actorsLength");
					if(actorsLength==1)
					{
					...
					else if(actorsLength>=2)
					{
					    ...
					    //fullActorOne= actors[0] +" "+ actors[1]; 
						actorOne=subStringFunction(fullActorOne,1,135);
			...
			// 是连续剧父集
			int fathervodid=((Integer)filmInfoMap.get("FATHERVODID")).intValue();
			
			
			
			String fatherDefinition="";
			if(fathervodid!=-1)
			{
			     Map FatherfilmInfoMap = metaData.getVodDetailInfo(fathervodid); 
				 if(FatherfilmInfoMap != null && FatherfilmInfoMap.size() != 0)
				 {
				      // 片花ID
			          assessId = ((Integer)FatherfilmInfoMap.get("ASSESSID")).intValue(); 
			          //是否包含片花：1：包含	0：不包含
			          hasAssess= ((Integer)FatherfilmInfoMap.get("ISASSESS")).intValue();
					  //高清支持
					  fatherDefinition=FatherfilmInfoMap.get("DEFINITION").toString().trim();
				 }
			}
			if("".equals(fatherDefinition))
			{
			    fatherDefinition=definition;
			}
			...
			if ( isSitCom == 1 )
			{
				...
				if(countTotal > 0)
				{
					...
					for(int i = 0; i < fetchNum; i++ )
					{
						...
						//子集Id
						String vodId = String.valueOf((Integer)sitcomMap.get("VODID"));
...
%>
		...
		var vodObject = new Object();
			...
			//连续剧父级ID
            vodObject.fathervodid = "<%=fathervodid%>";
			//是否被收藏
			...
			//父级VOD是否支持高清
			vodObject.fatherDefinition = "<%=fatherDefinition%>";
			//VOD的下线时间
