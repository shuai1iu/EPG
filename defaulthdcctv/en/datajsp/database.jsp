...
<%!public class MyUtil {
		...
		public ArrayList getListData(String code, int size, int curpage,
			...
			else if(mytype.equals("vas"))
			{
				...
				//将vasurl以“VASURL”这一名称存入记录VAS信息的对象中
			...
			else if (mytype.equals("type"))
			{
				...
				for(int i=0;i<templist.size();i++)
				{
					HashMap temp1=(HashMap)templist.get(i);
					HashMap temp2=(HashMap)temp1.get("POSTERPATHS");
					String[][] shuzu=new String[temp2.size()][];
					if(!temp2.isEmpty())
				    for(int j=0;j<temp2.size();j++)
				    {
				    	if(temp2.containsKey(""+j))
				    	{
				        	String[] temp3 = (String[])temp2.get(""+j);
				    	    shuzu[j] = temp3;
				    	}
				    }
				    temp1.put("POSTERPATHS",shuzu);
				}
				list.set(1,templist);
			}
