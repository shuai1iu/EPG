...
<%@ include file="datajsp/affectionalFilmData.jsp"%>
...
window.onload = function()
{
	...
	area2.areaOkEvent = function()
	{
		...
		{
			window.location.href="detail.jsp?vodid="+temppageTurnList[area2.curindex].VODID+"&cateNamePosition=3&returnurl="+escape(location.href)+"&typeid=<%=aiqing%>";
			saveFocstr(pageobj);
		}
	}
	...
	area2.asyngetdata=function()
	{
		...
		$('hidden_frame').src = "<%=basePath%>datajsp/affectionalFilmData.jsp?isFirstFlag=1&stratPosition="+(this.curpage-1)*8;
		//getAJAXData("datajsp/affectionalFilmData.jsp?stratPosition="+(this.curpage-1)*8+"&isFirstFlag=1",bindFilmsData);
		
	}
	
	area1 = AreaCreator(1,4,new Array(0,-1,2,-1),"area1_list_","className:item item_focus","className:item");
	area1.stablemoveindex = new Array("0-3,1-3,2-3,3-3",-1,-1,-1);
	area1.areaOkEvent = function()
	{
		...
		else if(area1.curindex==3)
		{
			   ...
			   &&area1_numString>0&&area1_numString<=area2.pagecount)
			{
			    ...
			    $('hidden_frame').src = "<%=basePath%>datajsp/affectionalFilmData.jsp?isFirstFlag=1&stratPosition="+(area2.curpage-1)*8;
			}
			else
			{
				area1_numString="";
	...
	pageobj = new PageObj(areaid!=null?parseInt(areaid):0,indexid!=null?parseInt(indexid):3, new Array(area0,area1,area2,area3),null);
	...
	{
	    bindFilmsData(affectionalFilmData);
	}
}
...
<body>
	...
	<!--menu-->
		<div class="menu">
        	<div class="item item_select" style="z-index:-2;left:456px;"></div>
			<!--焦点 
