...
<%@ include file="datajsp/otherFilmData.jsp"%>
...
window.onload = function()
{
	...
	area2.areaOkEvent = function()
	{
		...
		{
			window.location.href="detail.jsp?vodid="+temppageTurnList[area2.curindex].VODID+"&cateNamePosition=5&returnurl="+escape(location.href)+"&typeid=<%=qita%>";
			saveFocstr(pageobj);
		}
	}
	...
	area2.asyngetdata=function()
	{
		...
		$('hidden_frame').src = "<%=basePath%>datajsp/otherFilmData.jsp?isFirstFlag=1&stratPosition="+(this.curpage-1)*8;
		//getAJAXData("datajsp/otherFilmData.jsp?stratPosition="+(this.curpage-1)*8+"&isFirstFlag=1",bindFilmsData);
		
	}
	
	area1 = AreaCreator(1,4,new Array(0,-1,2,-1),"area1_list_","className:item item_focus","className:item");
	area1.stablemoveindex = new Array("0-5,1-5,2-5,3-5",-1,-1,-1);
	area1.areaOkEvent = function()
	{
		...
		else if(area1.curindex==3)
		{
			   ...
			   &&area1_numString>0&&area1_numString<=area2.pagecount)
			{
			    ...
			    $('hidden_frame').src = "<%=basePath%>datajsp/otherFilmData.jsp?isFirstFlag=1&stratPosition="+(area2.curpage-1)*8;
			}
			else
			{
				area1_numString="";
	...
	pageobj = new PageObj(areaid!=null?parseInt(areaid):0,indexid!=null?parseInt(indexid):5, new Array(area0,area1,area2,area3),null);
...
<body>
	...
	<!--menu-->
		<div class="menu">
        	<div class="item item_select" style="z-index:-2;left:760px;"></div>
			<!--焦点 
