...
function videoControl(keyval)
{
	switch(keyval)
	{
		case <%=KEY_BOTTOMLINE%>://直播号下划线
			...
			break;
		case <%=KEY_0%>:
			if(PauseDivIsShow){showInputTime(0);}
			else{pressKeyNum(0);}
			break;
		case <%=KEY_1%>:
			if(PauseDivIsShow){showInputTime(1);}
			else{pressKeyNum(1);}
			break;
		case <%=KEY_2%>:
			if(PauseDivIsShow){showInputTime(2);}
			else{pressKeyNum(2);}
			break;
		case <%=KEY_3%>:
			if(PauseDivIsShow){showInputTime(3);}
			else{pressKeyNum(03);}
			break;
		case <%=KEY_4%>:
			if(PauseDivIsShow){showInputTime(4);}
			else{pressKeyNum(4);}
			break;
		case <%=KEY_5%>:
			if(PauseDivIsShow){showInputTime(5);}
			else{pressKeyNum(5);}
			break;
		case <%=KEY_6%>:
			if(PauseDivIsShow){showInputTime(6);}
			else{pressKeyNum(6);}
			break;
		case <%=KEY_7%>:
			if(PauseDivIsShow){showInputTime(7);}
			else{pressKeyNum(7);}
			break;
		case <%=KEY_8%>:
			if(PauseDivIsShow){showInputTime(8);}
			else{pressKeyNum(8);}
			break;
		case <%=KEY_9%>:
			if(PauseDivIsShow){showInputTime(9);}
			else{pressKeyNum(9);}
			break;
		case <%=KEY_BTV%>:
		case <%=KEY_NVOD%>:
