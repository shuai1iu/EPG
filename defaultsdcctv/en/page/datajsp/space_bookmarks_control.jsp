...
function loadBody() {
	...
	//构建菜单区域
     if(collentNum==0)
     {
        area0 = AreaCreator(4, 1, new Array(-1, -1, -1, 3), "area0_list_", "className:onboder", "className:offboder");
	 }
	 else
	 {
        area0 = AreaCreator(4, 1, new Array(-1, -1, -1, 1), "area0_list_", "className:onboder", "className:offboder");
	}
   ...
   //构建删除列表区域
   area2 = AreaCreator(7, 1, new Array(-1,1, -1, 3), "area2_list_", "className:onboder", "className:offboder");
   ...
   {  
    ...
    if(collentNum==0)
    {
       area3 = AreaCreator(3, 1, new Array(-1,0, -1, -1), "area3_list_", "className:link onboder", "className:link");
	}
	else
	{
	   area3 = AreaCreator(3, 1, new Array(-1,2, -1, -1), "area3_list_", "className:link onboder", "className:link");
	}
    popList=new Array();
	
	//构建删除操作层
     area4=AreaCreator(1,2,new Array(-1,-1,-1,-1),"area4_list_","className:link onboder","className:link offboder");
	...
	delSuPop=new Popup("area5_list",new Array(area4),0,0);
	area5.doms[0].domOkEvent=delSuFun;
	//end
	
	//构建删除失败层
    area6=AreaCreator(1,1,new Array(-1,-1,-1,-1),"area6_list_","className:link onboder","className:link offboder");
	...
	if(collentNum==0){
         pageobj = new PageObj(0, 0, new Array(area0, area1,area2,area3),popList);
	}
	else
	{    
...
function showTj(data){
	...
	{    
		...
		if(i<data.length) 
	    {
		   ...
		   area3.doms[i].updateimg("../"+data[i].POSTERPATHS.type5[0]!=undefined?"../"+data[i].POSTERPATHS.type5[0]:'images/no_picture_259x165.jpg');
		   //0:视频VOD、1:视频频道、2:音频频道、3:频道、4:音频AOD、10:VOD、100:增值业务、300:节目、9999:混合栏目
		   area3.doms[i].setcontent("",data[i].VODNAME,12);
		   area3.doms[i].mylink="vod_turnpager.jsp?vodid="+data[i].VODID+"&typeid=<%=shouyetuijian1code%>"+"&returnurl="+returnurl;
...
function showCollectDel(data,count){		 
	 ...
	 area1.lockin=false;
	 area2.lockin=false;
	 $("pageArea").innerHTML=area1.curpage+"/"+pagecount;
}
