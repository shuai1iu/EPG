...
<%
String ADPicURL="";
//indexADFlag 是开机广告的标识 1或null说明需要加载广告 3不需要加载广告
	    ...
	    {//使用Callable接口作为构造参数   
		 ...
		 public ArrayList call() {   
           //真正的任务在这里执行，这里的返回值类型为String，可以为任意类型   
				 try{
					tmpAdVasInner =(ArrayList)new MetaData(requestInner).getVasListByTypeId("10000100000000090000000000038451",6,0);
					  } catch(Exception e)
					  {
...
System.out.println("=======================tag====================================="+indexADFlag);
%>
<style type="text/css">
body {
background:  url("../<%=ADPicURL%>") no-repeat;
}
...
{ 
	//“index”是首页主体部分div的id ，“ad”则是广告覆盖的div的id
	
	document.getElementById("index").style.visibility="hidden";
	if(ADPicURL!=null&&ADPicURL!=""&&ADPicURL!=undefined&&indexADFlag!=ADUnload)
	{
	...
	//此处是为了处理EC1308A盒子的不识别visibility 属性而作的判断
	document.getElementById("index").style.display= STBType=="EC1308_pub" ? "none" : "inline";
}	
...
{	
	...
	mp1.setMuteFlag(0);
    //    mp1.setMuteUIFlag(1);
	//利用ajax改变session中的的indexADFlag绑定值
	if(indexADFlag==ADLoading)
	{
		var xhr = getXhr();
		xhr.open('post',
				'changeSession.jsp',
   ...
   function VASimg1(data,area)
{
	...
	if(area.datanum >= 0)
	{
		...
		area.doms[0].mylink = "../../../"+data[0].VASURL+"?returnurl="+returnurl;			
	/**
...
<div id="ad" style="display:inline;">
	<img id="frame_content"  src="../<%=ADPicURL%>" style="width:101%;height:101%" onload="iFrameHeight()"   onerror="err()"/>
</div>
