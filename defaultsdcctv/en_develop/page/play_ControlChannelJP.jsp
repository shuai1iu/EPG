...
<%@ include file="play_ControlChannelName.jsp"%>
...
.live-item .con .item-back {
	left:145px; 
	position:absolute; 
	top:0;
}
.live-item .con .item-back,
.live-item .con .item-back .link,
.live-item .con .item-back .link img,
.live-item .con .item-back .txt {
	height:39px;
	width:45px;
}
.live-item .con .item-back .txt {
...
var NumActionOn = 0;//20130916 18:00 ZSZ 直播号OK键处理
var jumpNum = 0;//20130916 18:00 直播号OK键跳转频道号
var showchannelName = "";//20130916 18:00 当前频道名称
...
{	
	...
	/*********20130911 15:38 ZSZ 新增显示频道名称**********/
	showChannelName(chanNum);
	/*********20130911 15:38 ZSZ 新增显示频道名称**********/
...
function loadData()
{
	filmInfo.location.href = "play_ControlChannelListTvJP.jsp?CHANNELID="+channelId+"&CHANNELNUM="+currChannelNum+"&COMEFROMFLAG="+comeType+"&isfromgat="+isfromgat;
}	
//向下键
function arrowDown()
{
...
{	
	...
	miniInfo.location.href = "play_ControlChannelminiInfo.jsp?CHANNELID=" + channelIds[currChannelIndex] + "&pltvStatusFlag=" + isSubPreview[currChannelIndex] + "&CHANNELNUM=" + currChannelNumStr;
	/*********20130905 17:30 ZSZ 增加OSD显示频道号************/
...
{	
	...
	if(0 == bottomLineNum){
		showChannelNum();
		showChannelName();//20130917 9:50 ZSZ 增加显示频道号时显示频道名称
	}//直播下划线存在的时候不显示
	
	if(jumpDivIsShow){hideJumpTimeDiv();}
	if(volumeDivIsShow){hideBottom();}
	if(seekDivIsShow){$("seekDiv").style.display = "none";}
	$("filmInfo1").style.visibility = "visible";
	infoFlag = "true";
	if(hiddenTimer){clearTimeout(hiddenTimer);}//20130916 16:50 ZSZ 尝试修复计时器bug
...
{		
	...
	chaninfo += "<div class='item-back' style='top:"+top+"px'><div class='link'><a id='achan_"+j+"' href='#'><img src='../images/t.gif'/></a></div><div id='tvodosd_"+j+"' class='txt'>回看</div></div>";
		}else{
			//alert("-------------------"+channelNumsShow[index]);
			...
			chaninfo += "<div class='item-back' style='top:"+top+"px'><div class='link'><a id='achan_"+j+"' href='#'><img src='../images/t.gif'/></a></div><div id='tvodosd_"+j+"' class='txt'></div></div>";
		}
	}
	...
	for(var j = 0; j<rows; j++){
                ...
                if(index <= channelNames.length-1 && index <= channelNumsShow.length-1)  //修改中兴全屏播放频道列表出现undefined的问
         		 { 
				    ...
				    $("tvodosd_"+j).innerHTML="回看";
				 }
				else{
					 $("osd_"+j).innerHTML="&nbsp;";
					 $("tvodosd_"+j).innerHTML="&nbsp;";
				}
	}
...
function freetvodFocus(curr_focus)
{
	var focusAt = curr_focus%rows;
       if($("achan_"+focusAt) != undefined)  //ztewebkit
	$("achan_"+focusAt).blur();
}	
//获得焦点选择
...
function settvodFocus(curr_focus)
{
	var focusAt = curr_focus%rows;
	$("achan_"+focusAt).focus();
}	
function gotoEnd()
{
...
{	
	//OK键直接跳转频道
	if(1 == NumActionOn && 0 != jumpNum){
		playByChannelNum(jumpNum);
		return;
	}
	...
	//false:已经显示
	    ...
	    if(!istvod){
			...
			var instanceId = mp.getNativePlayerInstanceID();
			mp.releaseMediaPlayer(instanceId);
			window.location.href="playbackJP.jsp?returnurl="+escape(window.location.href);
		}
	}else if(jumpDivIsShow && jumpPos==4){
...
function showFirstChannelList()
{
	...
	showChannelListDiv();
	clearTimeout(channelListTimer);
	channelListTimer = setTimeout(hiddenChannelList,3000);
}	
//获取正整数
function getInt(num)
{
...
function channelNumAction(num)
{
	...
	/**********20130828 15:16 ZSZ 修改频道号显示样式**********/
	var numText = "";

	if(num < 10){
		numText = "00" + num.toString();
	}else if(num < 100){
...
function inputNum(i)
{
	//直播号ok键跳转开启
	NumActionOn = 1;
	if(quitDivIsShow||"false"==getDataFlag||number * 10 >= 1000||oneKeySwitchJumpInfoIsShow||"false" == channelListFlag){return;}		
	...
	if(numCount >= 3||jumpDivIsShow||seekDivIsShow){return;}
	numCount++;
	tempNumber = tempNumber+""+i;
	number = number * 10 + i;
	jumpNum = number;
	channelNumAction(number);//20130911 15:19 ZSZ 规避显示多个0
	clearTimeout(timeID);
...
* Time:2013.9.4 9:50
* Author:ZSZ
* description:增加显示、隐藏频道栏目名称功能
* function定义了显示隐藏频道名称的方法
*********************/

//显示频道名称
function showChannelName(num){
	if(-1 != num){
		for (var i = 0;i < showchannelNums.length;i++){
			if(showchannelNums[i] == num ){
				showchannelName = showchannelNames[i];
			}
		}
	}else{
		hideChannelName();
	...
	//空值保护
	if('undefined' != showchannelName && null != showchannelName && ! "".equals(showchannelName)){
		document.getElementById("topframe_channelName").innerHTML = showchannelName;
		document.getElementById("topframe_channelName").style.display = "block";
	}
}
//隐藏频道名称
function hideChannelName(){
	document.getElementById("topframe_channelName").style.display = "none";
}
...
<div id="showMessageDiv" style="position:relative; background-color:#eaeaea; float:left; height:60px; width:338px; left:120px; top:455px; text-align:center;z-index:11; display:none;">
...
<%--直播节目单OSD---------ztewebkit---------------------------------%>
<div id="filmInfo1" style="position:absolute; left:420px; top:0px; width:360px; height:530px;z-index:3; visibility:hidden;">
...
<div id="no_order" style="position:absolute; right:150;top:50px; width:350px; height:135px; display:none">
  <table width="340px" height="135px" border="0">
    ...
    <tr>
      <td id="orderInfo" width="340px" align="center" style="font-size:40px; color:#FFF;"></td>
    </tr>
    <tr>
      <td id="showmessage" width="340px" align="center" style="font-size:40px; color:#FFF;"></td>
    </tr>
    <tr>
      <td  height="40"></td>
    </tr>
...
<!-- 右上显示直播号-->
<div id="topframe" style="position:absolute;left:175px; top:8px; width:300px; height:50px; z-index:1; font-size:50px; text-align:center; color:#FFF;"></div>

<!--2013.09.04 ZSZ 增加频道名称-->
<div id="topframe_channelName" style="position:absolute;left:175px; top:60px; width:300px; height:40px; z-index:1; font-size:40px; text-align:center; color:#FFF; font-family:'微软雅黑'"></div>
<!--2013.09.04 ZSZ 增加频道名称-->
