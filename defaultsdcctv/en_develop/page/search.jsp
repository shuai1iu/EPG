...
function setValue(data,pageCount)
{
	...
	for(var i = 0 ; i < area6.doms.length; i ++)
	{
		if( i < data.length)
		{
			area6.doms[i].setcontent("",data[i].vodName,22,true,false); //20130905 hxt 修改了搜索结果过长换行显示问题
		
			area6.doms[i].url = "vod-tv-detail.jsp?vodid="+data[i].vodId ;
			//area6.doms[i].mylink="vod-tv-detail.jsp?vodid="+data[i].vodId + "&returnurl="+returnurl;
		}
		else
		{
			area6.doms[i].updatecontent("");
...
function initPage()
{
	...
	area0.areaOkEvent = function()
	{
                ...
                }else if($("words").innerHTML=="SY")
				{
					window.location.href = "shouying/page/index.jsp";
				}
				else if($("words").innerHTML=="F")
                {
			window.location.href = "vod_turnpager.jsp?catename=VBR&typeid=10000100000000090000000000041981&returnurl=index.jsp";
                }else if($("words").innerHTML=="A")
                {
              window.location.href = "jpui/page/newJpChannel.jsp?returnurl=../../index.jsp";}
                else if($("words").innerHTML=="W")
                {
                  window.location.href = "whxwzq/portal.jsp";}
                else if($("words").innerHTML=="WH")
                {
                  window.location.href = "whxwzq/epgPage/page/news.jsp";}
