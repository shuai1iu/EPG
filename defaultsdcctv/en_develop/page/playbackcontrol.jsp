			...
			if(pageCount!=0){
				...
				for (var k = 0; k < area2.doms.length; k++) {
					if (k < data.length) {
						...
						var url = "au_ReviewOrSubscribe.jsp?channelNumber=" +area0.doms[area0.curindex].channelNumber +"&PROGID=" + data[k].PROGRAMID + "&PLAYTYPE=4&CONTENTTYPE=300&&BUSINESSTYPE=5&PROGSTARTTIME=" + data[k].STARTTIME + "&PROGENDTIME=" + data[k].ENDTIME + "&ISSUB=1&PREVIEWFLAG=1&TVOD=1&CHANNELID=" + area0.doms[area0.curindex].dataurlorparam + "&returnurl="+escape("playback.jsp");
        ...
        function getChannelData(data, pageCount) {
            ...
            for (var i = 0; i < area0.doms.length; i++) {
                ...
                if (i < data.length) {
					...
					area0.doms[i].channelNumber=data[i].channelNumber;
                }
                else {
