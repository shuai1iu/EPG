...
function PageObj(curareaid,index,areas,popups)
{
   ...
   */
   ...
   if(this.areas!=undefined)
   {
	   ...
	   if(t_num!=undefined)
		clearTimeout(t_num);
		t_num=setTimeout(cruntAreaOnfocus,600);
   }
   
   function cruntAreaOnfocus()
   {
	   areas[curareaid].changefocus(index,true);
   }
   //设置弹出层
...
function Area(numrow,numcolum,doms,directions)
{
   ...
   */ 
   ...
   {
	   //执行异步
	   //这个url和dataurlorparam需要是动态的数据
	   var param=new Array();
	   param[0]=tmpfunction;
	   param[1]=dataurlorparam;
	   getAJAXData(url,param,this.mycallback); 
   };
   this.mycallback=function(responseText,param)
   {
	   //执行回调
	      getdata(responseText,param[1]);
	   //执行回调
	      var tmpfunction=param[0];
		  tmpfunction();
   }
   /** 
   ...
   */ 
   ...
   //屏蔽烽火盒子数组是空时长度值为undefined
	if(Area.names.length==undefined)
	{
		this.flag=0;
	}
	else
   ...
   */
   this.move=function(num,dirindex)
   {
	   ...
	   if(cross)
	   {
		  ...
		  {
			 /* if(this.numrow>1&&this.numcolum>1)
			  {
				  if(dirindex==0&&(this.curindex<=(this.numcolum-1)))
				  {
					 this.pageturn(-1); 
				  }
				  else if(dirindex==2&&(this.curindex>(this.numcolum-1)))
				  {
					 this.pageturn(1);
				  }
			  }
   ...
   */
   this.insidemove=function(circle,dirindex,num,cross)
   {
	   ...
	   if(!this.useoneobj)
		 ...
		 /* if(nextindex<0)
			  nextindex=0;
		  else if(nextindex>=this.datanum)
			  nextindex=this.datanum-1;*/
   ...
   */
   this.pageturn=function(num,pagebtn)
   {
	   if(this.curpage!=undefined&&this.pagecount!=undefined)
	   {
		   ...
		   {
		   if(nextpage>=1&&nextpage<=tmpthis.pagecount)
	       {
		       //如果通过移动到边缘翻页
		       if(!pagebtn)
			   {
		        if(tmpthis.crossturnpage)
			    {
			    	if(num>0)
			    	{
			    		tmpthis.changefocus(tmpthis.curindex,false);
		   ...
		   if(tmpthis.curpage==1&&!!tmpthis.firstpageEvent)
		     tmpthis.firstpageEvent();
		   else if(tmpthis.curpage==tmpthis.pagecount&&!!tmpthis.lastpageEvent)
		     tmpthis.lastpageEvent();
		   if(!!tmpthis.areaPageTurnEvent1)
	          tmpthis.areaPageTurnEvent1();
		   }
			  */
	       if(nextpage>=1&&nextpage<=this.pagecount)
	       {
		       ...
		       if(!pagebtn)
			   {
				  if(this.crossturnpage)
				  {
					  if(num>0)
					  {
			               ...
			               {
							   this.changefocus(this.curindex,false);
							   if(this.curpage==this.pagecount&&this.curpage>1)
							   {
								   this.changefocus(0,true);
								   
							   }
							   else if(this.pagecount>1&&this.curpage!=this.pagecount)
							   {
	       ...
	       else if(this.circledir==0)
	       {
			   ...
			   if(t1!=undefined)
		       clearTimeout(t1);
		       t1=setTimeout("Area.names['"+this.flag+"'].asyngetdata()",300);
		       //如果通过移动到边缘翻页
		       if(!pagebtn)
			   {
		        if(this.crossturnpage)
   ...
   */
   this.setpageturndata=function(datanum,pagecount)
   {
	   ...
	   if(this.islastpage)
	   {
	      this.changefocus(datanum-1,true);
	      this.islastpage=false;
	   }
...
function DomData(firstdomid,firstfocusstyle,firstunfocusstyle)
{
   ...
   */
	...
	*/
	...
	//屏蔽烽火盒子数组是空时长度值为undefined
	if(DomData.names.length==undefined)
	{
		this.flag=0;
	}
	else
	...
	this.changefocus=function(focusornot,backnochanggedata)
	{
		if(focusornot)
	   {
		   ...
		   if(this.cutcontent!=undefined)
	        this.updatecontent("<marquee scrollamount='4' style='height:34px'>"+this.content+"</marquee>");
		   //聚焦事件
		   if(!!this.focusEvent)
		      this.focusEvent();
	   }
...
function Popup(popupdom,areas,areaid,index)
{
    ...
    */
	...
	//静态属性
	//屏蔽烽火盒子数组是空时长度值为undefined
	if(Popup.names.length==undefined)
	{
		this.flag=0;
	}
	else
