...
<%@ include file="datajsp/keydefine.jsp" %>
<%@ include file="datajsp/config_playControl.jsp" %>
...
.winTxt {
        ...
        .playbar-c {
            background: url("../images/playbar-b.png") no-repeat;
            height: 80px;
            width: 640px;
            position: absolute;
            left: 0px;
        ...
        .progress-wrap {
            background: url("../images/posbar-361x16.png") 0px 6px no-repeat;
            height: 30px;
            width: 364px;
            position: absolute;
            left: 168px;
        ...
        .progress-wrap .progress-bar .progress-position-read {
            background: url("../images/posbar-read.png") 0px 0px no-repeat;
            height: 8px;
            position: absolute;
            top: 0px;
        }
        ...
        .progress-moveBox .pic {
            background: url("../images/posbar-sun.png") no-repeat;
            position: absolute;
            left: 43px;
            top: 0px;
        }
        .progress-moveBox .pic_focus {
            background: url("../images/posbar-sun_focus.png") no-repeat;
        }
...
var STBType = "";
if(Authentication!=undefined&&Authentication.CTCGetConfig!=undefined){
...
function showInputTime(id){
    ...
    if(jumpPos == 0){
		...
		if(bufInput == " "){bufInput = "";}
			...
			if(($("jumpTimeHour").innerHTML).length==2){pressRight();}
		}
    }else if(jumpPos == 1){
		...
		if(bufInput == " "){bufInput = "";}
			...
			if(($("jumpTimeMin").innerHTML).length==2){pressRight();}
		}       	
    }
}
//20120330修改删除输入的时间数字
function delInputTime()
{
    ...
    if(jumpPos == 0){
    	bufInput = $("jumpTimeHour").innerHTML;
		if(bufInput.length>0){$("jumpTimeHour").innerHTML=bufInput.substring(0,bufInput.length-1);}
    }else if(jumpPos == 1){
        bufInput = $("jumpTimeMin").innerHTML;
		if(bufInput.length>0){$("jumpTimeMin").innerHTML=bufInput.substring(0,bufInput.length-1);}
    }
}
...
function changeVolume(volume)
{
	...
	if(volume!=0)
	{
	    $("volumeLen").style.width=(volume/100)*320+"px";
	}
	else
	{
	    $("volumeLen").style.width=1+"px";
	}
}
//音量控制
function volumeUp11()
...
function fastRewind()
{
	...
	$("seekDiv").style.top = "450px";
	$("seekDiv").style.height = "80px"; 
	...
	jumpDivIsShow = false;
		...
		playStat = "fastrewind";
		shiftFlag=1;		
	if(playStat != "fastrewind"||(speed >= 32 && "fastrewind"==playStat)){speed = 1;} 
...
function displaySeekTable()
{
	...
	if(0==isSeeking){
	    ...
	    $("seekDiv").style.top = "400px";
	    $("seekDiv").style.height = "150px"; 
		$("seekDiv").style.display = "block"; //显示整个在暂停跳转DIV
		$("currentTime_progress").className="baron";
		currTime = mp.getCurrentPlayTime();
		processSeek(currTime);			
...
function processSeek(_currTime)
{
	...
	//alert(_currTime+"----"+mediaTime+"--"+timePerCell+"--"+currCellCount); 
	...
	if(currCellCount==0)
	{
		$("td_1").style.width = 1+"px";
	}
	else
	{
	    $("td_1").style.width = currCellCount * 3.6+"px";
	}
}
//检查进度条
...
{    	
	...
	if(checkJumpTime(inputValueHour, inputValueMin))
	{
		...
		var timeStamp = "" +  year + month + day + "T" + hour + mins + "00" + "Z";
...
function getShiftBeginTime()
{
	...
	var mins = beginTime.getMinutes();
	var sec = beginTime.getSeconds();
	if (sec >= 30) beginTime.setMinutes(mins + 1);   	
	var hour = beginTime.getHours();
	mins = beginTime.getMinutes();
	if (hour < 10) hour = "0" + hour;
	if (mins < 10) mins = "0" + mins;   
	return hour + ":" + mins;    	
}
function getShiftBeginTime2()
{
	...
	if (hour < 10) hour = "0" + hour;
	if (mins < 10) mins = "0" + mins;   
	var timeFlag=year+""+month+""+day+""+hour+""+mins+""+sec;
...
{	
	...
	if (sec >= 30) currTime.setMinutes(mins + 1);
...
function getCurrPlayTime(currPlayTime)
{
	...
	var mins = currPlayTime.getMinutes();
	if (sec >= 30){
	   mins = mins + 1;
	   if(mins == 60){ mins = 59;}
	}
...
function parseUtcTime(utcTime)
{
	...
	var mins = parseInt(utcTime.substr(11, 2));
	...
	if (mins == 0) mins = parseInt(utcTime.substr(12,1));
	if (sec == 0) sec = parseInt(utcTime.substr(14,1));	
	var d =  new Date(year, month -1, day, hour + 8, mins, sec);   
	return d;   	 	
}  
function resume()
{
...
{	
	...
	document.getElementById("testText").innerHTML=backUrl;
...
{
	jumpPos=0;
	$("jumpTimeHour").innerHTML="";$("jumpTimeMin").innerHTML="";	
...
function getInt(num)
{
	...
	return parseInt(currpage,rows);
}	
//根据索引判断当前是第几页
function realCurrPage()
{
	if(chanListFocus % rows == 0){currListPage = chanListFocus / rows;}
	else{currListPage = Math.floor(chanListFocus / rows);}
}	
//直播列表层显示
...
{
	if(hiddenDivTimer){clearTimeout(hiddenDivTimer);}
...
function countTime(currTime,count)
{
   ...
   var temDate = value2+'/'+value3+'/'+value1+' '+oldTime;
   var T =new Date(Date.parse(temDate));
   var t=Date.parse(T)+60*count*1000*3;   
	   T.setTime(t);
...
function formatDate(dateobj)
{
	...
	if (mins < 10){mins = "0" + mins;}
	if (sec < 10){sec = "0" + sec;}
	var dateStr = "" + year  + month  + day + hour  + mins  + sec;
	return dateStr;
}
...
<%--直播节目单OSD------------------------------------------%>
<!--<div id="filmInfo1" style="position:absolute; left:420px; top:0px; width:360px; height:530px;display:none;z-index:3;">
...
<div class="infordispaly" id="filmInfo1">
<iframe name="miniInfo" id="miniInfo" src="" scroll="no" height="530px" width="640px"></iframe>
</div>
...
<div id="seekDiv" class="play-page" style="position:absolute;width:640px;height:150px;left:5px;top:400px; z-index:1; display:none">
    <div id="processDiv" style="position:absolute;width:640px;height:80px;left:0px;top:0px;">
    <div id="statusImg" class="key" style="top:22px; left:10px;"></div>
    <div id="beginTime" class="timeline" style="position:absolute;left:88px;">00:00:00</div>
    ...
    <div id="fullTime" class="timeline" style="position:absolute;left:540px;">02:00:22</div>
    </div>
    <div class="set-time"  id="jumpTimeDiv" style="position:absolute; width:590px; height:57px;left:42px; top:75px;">
        <div style="position:absolute; top:6px; width:153px;">输入定位时间：</div>
        <div style="position:absolute;height:52px; width:78px;left:150px;" id="jumpTimeHour" class="inp"></div> <!--当前为 class="inp"；焦点为class="inpon"-->
        <div style="position:absolute;top:6px;left:235px;">时</div>
        <div style="position:absolute;left:265px;height:52px; width:78px" id="jumpTimeMin" class="inp"></div> <!--当前为 class="inp"；焦点为class="inpon"-->
        <div style="position:absolute;top:6px;left:350px;">分</div>
        <div class="pop_btns" style="top:8px; left:374px; width:213px; height:42px">
        <div style="position: absolute; widows:105px; left:0px;"><div id="ensureJump" class="btn02" style="left:0px;"><img src="../images/btn2_bg.png"></div><div style="position:absolute; left:0px;">跳转</div></div><!--焦点为class="btn02 on2"-->
        <div style="position: absolute; widows:105px; left:108px;"><div id="cancelJump" class="btn02" style="left:0px;"><img src="../images/btn2_bg.png"></div><div style="position:absolute; left:0px;">重置</div></div>
        </div>
    </div>
</div>
...
<div id="volumeDiv" class="volume" style="display:none">
    <div style="width:320px;"><div id="volumeLen" class="on" style="width:0px;"></div></div> <!--总宽度为320px;也可以用100%-->
...
<div id="testText" style="position:absolute; font-size:14px; color:#F00; left: 35px; top: 258px; width: 507px; height: 32px;">11111</div>
