...
<%@ include file="datajsp/vodSecondData.jsp"%>
<%
...
window.onload = function()
{
	  if(returnurl.indexOf('back=1')==-1)
	  {
	   ...
	   returnurl=escape(returnurl);
		...
		{
			var typeid = typeData[(area0.curpage-1)*10+area0.curindex].TYPE_ID;
			$('hidden_frame').src = "<%=basePath%>datajsp/vodTypeOrListData.jsp?isFirstFlag=1&stratPosition="+(area1.curpage-1)*8+"&typeID="+typeid;
		}
		for(i=0;i<8;i++)
		{
		...
		if("1"==backflag||"-1"==backflag)
		{
			...
			backfocus=getCurFocus("vodSecondindex");
			...
			else
			{
				...
				cruntpage=backfocus[0].curpage;
			
			}
		}	
	  pageobj=new PageObj(backfocus!=null?backfocus[0].areaid:0,backfocus!=null?backfocus[0].curindex:0,new Array(area0,area1));
	  pageobj.pageOkEvent=function()
	  {
	  
		var json=createAllFocstr(pageobj);
		saveCookie("vodSecondindex",json!=undefined?json:""); 
	  
	  }
	  
	  pageobj.goBackEvent=function()
	  {
		 window.location.href="vod.jsp?back=1";
	  }
	  $("cateName").innerHTML="点播-"+cateName;
	 turnpageTypesShowData(area0.curpage,typeData);
...
function bindTypesData(datavalue)
{
	...
	if(backfocus!=null&&isTurnPageFlag==0)
	{
		...
		var typeid = typeData[(backfocus[1].curpage-1)*10+backfocus[1].curindex].TYPE_ID;
		area1.curpage = cruntpage;
		$('hidden_frame').src = "<%=basePath%>datajsp/vodTypeOrListData.jsp?isFirstFlag=1&stratPosition="+(cruntpage-1)*8+"&typeID="+typeid;
	}
}
...
{
	var typeid = typeData[(area0.curpage-1)*10+area0.curindex].TYPE_ID;
	$('hidden_frame').src = "<%=basePath%>datajsp/vodTypeOrListData.jsp?isFirstFlag=1&stratPosition=0&typeID="+typeid;
}

function bindFilmsData(datavalue,contentTotalList)
{
	area1.pagecount = Math.ceil(contentTotalList/8);
	//var start = (area1.curpage-1)*8;
	//var size = (contentTotalList-start)>=8?8:(contentTotalList-start);
	...
	for(i=0;i<area1.doms.length;i++)
	{
	   if(i<datavalue.length)
	   {
		   ...
		   $("area1_textDiv_"+i).style.display = "block";
		   if(datavalue[i].ISSITCOM==0)
		   {
		       area1.doms[i].mylink="vod-tv-detail.jsp?catename="+cateName+"-"+typeData[(area0.curpage-1)*10+area0.curindex].TYPE_NAME+"&vodid="+datavalue[i].VODID+"&returnurl="+returnurl;
		   }
		   else if(datavalue[i].ISSITCOM==1)
		   {
			   area1.doms[i].mylink = "vod-zt-detail.jsp?catename="+cateName+"-"+typeData[(area0.curpage-1)*10+area0.curindex].TYPE_NAME+"&vodid="+datavalue[i].VODID+"&returnurl="+returnurl;
		   }
		   else
		   {
			   var typeid = typeData[(area0.curpage-1)*10+area0.curindex].TYPE_ID;
			   area1.doms[i].mylink = "vodThird.jsp?titleName="+cateName+"-"+typeData[(area0.curpage-1)*10+area0.curindex].TYPE_NAME+"&vodid="+datavalue[i].VODID+"&fatherTypeid="+fatherTypeid+"&cruntTypeId="+typeid+"&fatherTypeName="+cateName+"&areaid=0&indexid="+(((area1.curpage-1)*8+i)%10)+"&curpage="+Math.ceil(((area1.curpage-1)*8+i+1)/10);
		   }
	   }
	   else
	   {
		   area1.doms[i].mylink="#";
		   area1.doms[i].updateimg("../images/t.gif");
		   $("area1_textDiv_"+i).style.display = "none";
	   }
	}
...
<div class="wrapper">
	<!--head-->
	<div class="txt txt-title" style="top:5px; width:400px;" id="cateName">点播</div>
    ...
    <div class="list-c">
		<div class="item">
			...
			<div class="txt-wrap" id="area1_textDiv_0">
		...
		<div class="item" style="left:130px;">
			...
			<div class="txt-wrap" id="area1_textDiv_1">
		...
		<div class="item" style="left:260px;">
			...
			<div class="txt-wrap" id="area1_textDiv_2">
		...
		<div class="item" style="left:390px;">
			...
			<div class="txt-wrap" id="area1_textDiv_3">
		...
		<div class="item" style="top:187px;">
			...
			<div class="txt-wrap" id="area1_textDiv_4">
		...
		<div class="item" style="left:130px;top:187px;">
			...
			<div class="txt-wrap" id="area1_textDiv_5">
		...
		<div class="item" style="left:260px;top:187px;">
			...
			<div class="txt-wrap" id="area1_textDiv_6">
		...
		<div class="item" style="left:390px;top:187px;">
			...
			<div class="txt-wrap" id="area1_textDiv_7">
	...
	<div id="pageNum" class="txt txtDate02" style="position:absolute; left: 489px; top:475px; width: 149px; font-size:20px; text-align:center"></div>
</div>	
