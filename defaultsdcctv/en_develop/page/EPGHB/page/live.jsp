...
%> 
    ...
    <style type="text/css">
		...
		body {
			...
			top: 0px;
			color: #f1f1f1;
            background: url("../images/body-page-live.png") no-repeat;
...
<script type="text/javascript">
   ...
   window.onload=function()
   {
       ...
       getdata(list,pagecount);
	   //document.getElementById('hidden_frame').src="datajsp/liveasyndata.jsp?channelid="+area1.doms[area1.curindex].dataurlorparam;
	   //load_iframe()
	...
	function setTimePlay()
	{
		...
		{
		    playPage.mp.joinChannel(totallist[(area1.curpage-1)*8+area1.curindex].UserChannelID);
			playPage.mp.setVideoDisplayArea(263,87,355,266); 
	        playPage.mp.setVideoDisplayMode(0);
	        playPage.mp.refreshVideoDisplay();
		}
		var url = "<%=basePath%>datajsp/liveasyndata.jsp?channelid="+area1.doms[area1.curindex].dataurlorparam;
		...
		for(i=0;i<area1.doms.length;i++)
			{
				if(i<data.length)
			   {
				   var tmpSeq = "" + (data[i].UserChannelID);
            	   var tmpStr = "";
           		   if (tmpSeq.length == 1)
                		tmpStr = "00";
            	   if (tmpSeq.length == 2)
				   ...
				   area1.doms[i].numdom.innerHTML=tmpStr +(parseInt(data[i].UserChannelID));
			       area1.doms[i].dataurlorparam=data[i].ChannelID;
				   area1.doms[i].channelnum=data[i].UserChannelID;
				   area1.doms[area1.curindex].channelnum
				   if(data[i].ISTVOD=='0')
				   {
	   ...
	   for(i=0;i<area2.doms.length;i++)
			{
				if(i<data.length)
			   {
				   ...
				   area2.doms[i].setcontent("",data[i].proname,15);
				   area2.doms[i].datedom.innerHTML=time;
				   //if(parseInt(data[i].STATUS)==1)
			   }
			   else
			   {
				  area2.doms[i].updatecontent("暂无节目");
				  area2.doms[i].datedom.innerHTML="";
			   }
			}
   }
   //视频窗口
   function load_iframe()
	{
		...
		{
			playPage.location.href = "PlayTrailerInVas.jsp?left=263&top=87&width=355&height=266&type=CHAN&value=41&mediacode=41&contenttype=1&liveid="+area1.doms[area1.curindex].channelnum;
		}
		else 
		{
...
<!--pagebg-->
<!--<div class="pagebg" style="z-index:1">
	<div class="pic"><img src="../images/body-page-live.gif" width="640" height="530" /></div>
</div>-->
<!--pagebg the end-->
<div style="position:absolute; left:0px; height:0px;height: 530px;width: 640px;">
    <!-- S 面包屑 -->
    <div class="mod-bread" >
        直播中国
    </div>
...
<div class="item-arr-down" style="top:444px;">
    ...
    <!-- S 直播视频窗口 -->
    <div style="position:absolute; width:355; height:266; left: 263px; top: 92px;">
    <!-- 视频窗口 -->
...
<!--<iframe id="playPage" name="playPage" frameborder="0" height="266px" width="355px"></iframe>-->
    </div>
    <!-- E 直播视频窗口 -->
