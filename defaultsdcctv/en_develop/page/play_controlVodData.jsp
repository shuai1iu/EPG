...
String director = "暂无";//导演
String actor = "暂无";//演员
String introduce = "暂无";//介绍信息
...
if(!("-1".equals(fatherId)) && fatherId !="" && fatherId != null && fatherId != "null")
{	
	vodInfoMap = metaData.getVodDetailInfo(iFatherId);
}
else{vodInfoMap = metaData.getVodDetailInfo(iProgId);}
if(vodInfoMap != null)
{	
	vodName = (String)vodInfoMap.get("VODNAME");
	String actorOne = "暂无";
	String fullActorOne="暂无";
	String actorTwo = "暂无";
	String fullActorTwo = "暂无";
	int actorsLength=0;
	HashMap allActor = (HashMap)vodInfoMap.get("CASTMAP");
	director = (String)vodInfoMap.get("DIRECTOR");

	if(allActor!=null && allActor.size() != 0)
	{
		String[] actors=(String[])allActor.get(6);
	...
	if(actorOne.charAt(actorOne.length()-1)=='、'||actorOne.charAt(actorOne.length()-1)=='，')
		actorOne = actorOne.substring(0,actorOne.length()-1);
	actor = actorOne;
...
if(!"-1".equals(fatherId))
{
	vodName = (fatherVodName == null) ? "" : subStringFunction(fatherVodName,1,190);
	if(sitNum!=""&&sitNum!=null)
		vodName = vodName + "&nbsp;(" + sitNum + ")";
}
else
{
	vodName = (vodName == null) ? "" : subStringFunction(vodName,1,190);
}
actor = (actor == null) ? "暂无" : subStringFunction(actor,1,130);
director = (director == null) ? "暂无" : subStringFunction(director,1,130);
introduce = (introduce == null) ? "暂无简介" : subStringFunction(introduce,2,580);
/**************************对节目信息进行截取 end***********************************/	
%>
<script>
...
var vodName = "&nbsp;&nbsp;当前："+"<%=vodName%>";//本vod的名字
var director = "【导演】"+"<%=director%>";//导演
var actor = "【主演】"+"<%=actor%>";//演员
...
parent.createMinEpg();
//parent.createQuitDiv();//20120330屏蔽
</script>