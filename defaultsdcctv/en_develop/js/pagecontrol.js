...
function PageObj(curareaid,index,areas,popups)
{
   ...
   */
   ...
   {       
           if(this.areas[i] !=undefined)  //ztewebkit
           {
           this.areas[i].id=i;
   ...
   this.changefocus=function(areaid,index)
   {
	   ...
	   for(i=0;i<this.areas.length;i++)
       {
	     if(this.areas[i] !=undefined)  //ztewebkit	   
		   ...
		   if(i==areaid&&this.areas[i] !=undefined)//ztewebkit
...
function DomData(firstdomid,firstfocusstyle,firstunfocusstyle)
{
   ...
   */
	...
	this.changestyle=function(newstyle)
	{
	   ...
	   {
		   if(this.dom[j] ==undefined)  //ztewebkit
                      continue;   //ztewebkit
                   tmpstyle=newstyle[j];
    ...
    */
	...
	{
		if(this.mylink!=undefined&&this.mylink!="")  //ztewebkit
...
function Popup(popupdom,areas,areaid,index)
{
    ...
    */
	...
	{
		//此举专为深圳PPV添加，作用：使弹出层的焦点始终在0位置
		this.areas[this.curareaid].clearfocus();
		this.areas[this.curareaid].changefocus(0,true);
...
function getAJAXData(url, successMethed,param) {
    ...
    if (_in_ajax.readyState == 4) { if (_in_ajax.status == 200) { 
